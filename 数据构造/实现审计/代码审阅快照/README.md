...
- 当前快照日期：2026-08-28
- 当前 CPU/static 实现版本：`controlled_multi_future_runtime_v3`；科学设计版本仍为 `controlled_multi_future_f1_f4_v1_2`。
...
- `root_orchestrator.py`：pristine capture、三份 disposable feasibility scenes、freeze once、三份 fresh rollout scenes 与 3/3 finalizer 的 root-level contract；真实 SAPIEN adapter 尚未实现；
...
- `tests/controlled_multi_future/`：CPU static/pipeline contract tests（当前 45 tests）。
...
运行证据、截图、receipts 和 realized NPZ traces 不复制到本目录，统一位于相邻的 `../probe_outputs/`。Runtime-v2 实际结果见 `../runtime_v2_bounded_probe_execution_report_20260828.md/json`；runtime-v3 CPU proposal/registry/readiness 与 raw/root synthetic evidence 位于相邻目录。Stage 0 仍为 `BLOCKED_WITH_REASONS`。
