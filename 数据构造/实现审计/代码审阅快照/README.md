...
- 当前快照日期：2026-08-28
- 当前 GPU-preflight CPU/static 实现版本：`controlled_multi_future_runtime_v3_1`，current revision=`runtime_v3_1_cpu_hardening_v4`；科学设计版本仍为 `controlled_multi_future_f1_f4_v1_2`。
...
- `a0_orchestrator_v1_1.py`：adapter-agnostic 的 1 pristine + 3 fresh、zero planner/action、current/anchor/cleanup fail-closed A0 Gate；
...
- `tests/controlled_multi_future/`：CPU static/pipeline contract tests（当前 88 tests，其中 A0 5 tests）。
