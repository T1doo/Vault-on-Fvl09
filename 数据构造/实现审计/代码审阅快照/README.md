...
- 当前快照日期：2026-08-28
- 当前 GPU-preflight CPU/static 实现版本：`controlled_multi_future_runtime_v3_1`，current revision=`runtime_v3_1_cpu_hardening_v5`；科学设计版本仍为 `controlled_multi_future_f1_f4_v1_2`。
...
- `a0_activity_monitor_v2.py`、`a0_orchestrator_v1_2.py`：post-setup 独立 entry-point/physics monitor、per-scene artifact/hash 与 mismatch diagnostics；
...
- `runtime_v3_1_budget_v1_1.py`：六个未来非正式 scope 的机器预算；当前只有 A0 requestable；
- `probes/runtime_v3_1_authorization_v1_1.py`、`probes/gpu_guard_v2_1.py`：代码/预算/family/seed/spec/output/command绑定、expiry、one-shot consumption与 atomic GPU lifecycle；
- `real_sapien_adapter_v1_1.py`、旧 authorization/guard/A0 v1_1：只作历史/兼容 helper，不是 current execution entry；
- `family_runners_v3_1.py`、`family_repair_orchestrator_v1_1.py`：F1 root runner 与 F2/F3/F4 repair runner；尚未获得 GPU runtime evidence；
...
- `tests/controlled_multi_future/`：CPU static/pipeline contract tests（当前 131 tests）。
...
运行证据、截图、receipts 和 realized NPZ traces 不复制到本目录，统一位于相邻的 `../probe_outputs/`。Runtime-v2 实际结果见 `../runtime_v2_bounded_probe_execution_report_20260828.md/json`；runtime-v3_1 current synthetic evidence 为 `../probe_outputs/nonformal_root_pipeline_dry_run_runtime_v3_1_20260829_cpu10/`。A0待审批请求见相邻的 `../A0_USER_APPROVAL_REQUEST_RUNTIME_V3_1_V5.md/json`。真实 A0／family GPU evidence仍为0，Stage 0仍为 `BLOCKED_WITH_REASONS`。
