...
from controlled_multi_future.runtime_v3_4_1_scope_bundle_v1 import (
    PARENT_AUTHORIZATION,
    load_parent_user_authorization,
)
from controlled_multi_future.runtime_v3_4_1_scope_specs_v1 import planned_scope_spec


class RuntimeV341AuthorizationV1Test(unittest.TestCase):
    def build(self, directory):
        directory = Path(directory)
        scope = "F2_inside_targeted_v11"
        ...
        command = ["python", "-m", "controlled_multi_future.probes.runtime_v3_4_1_scope_runner"]
        ...
        receipt = {
            ...
            "authorization_id": "test-runtime-v3-4-1-auth",
            "authorized_run_id": "test-runtime-v3-4-1-run",
            ...
            "implementation_source_sha256": current_source_bindings_v3_4_1()["implementation_source_sha256"],
    ...
    def test_valid_authorization_binds_source_budget_gpu0_only_and_parent(self):
        root = Path("/nfs_share/lijunhui/Robotwin2/tmp")
        root.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(dir=root) as directory:
            receipt, now = self.build(directory)
            value = validate_authorization_v3_4_1(
                receipt,
                requested_scope="F2_inside_targeted_v11",
                now=now,
                expected_family="F2",
            ...
            self.assertEqual(value["allowed_physical_gpu_indices"], [0])
            self.assertFalse(value["stage0_authorized"])
            tampered = copy.deepcopy(receipt)
            tampered["allowed_physical_gpu_indices"] = list(range(8))
    ...
    def test_consumption_is_bound_to_v3_4_1_authorization(self):
        root = Path("/nfs_share/lijunhui/Robotwin2/tmp")
        root.mkdir(parents=True, exist_ok=True)
        with tempfile.TemporaryDirectory(dir=root) as directory:
            receipt, _ = self.build(directory)
            consumption = {
                "schema_version": "cmf_runtime_v3_4_1_authorization_consumption_v1",
