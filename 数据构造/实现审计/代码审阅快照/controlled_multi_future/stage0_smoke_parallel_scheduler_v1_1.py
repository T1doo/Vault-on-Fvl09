"""Pure-CPU assignment of four Stage 0 v1.1 jobs to fresh-idle GPUs."""
...
SCHEMA_VERSION = "cmf_stage0_smoke_parallel_schedule_v1_1"
...
def _idle(snapshot: Mapping[str, Any]) -> bool:
    try:
        ...
        return (
            ...
            and str(snapshot["uuid"]).startswith("GPU-")
            and 0 <= age <= 60
...
def assign_stage0_scopes_to_idle_gpus(
    ...
    if not isinstance(bundles, Mapping) or set(bundles) != set(STAGE0_SCOPES):
        raise ValueError("Stage 0 v1.1 scheduler requires the exact four-bundle set")
    idle = sorted(
    ...
    for scope, gpu in zip(STAGE0_SCOPES, idle):
        ...
        if bundle.get("scope") != scope or bundle.get("family") not in (
            "F1",
            "F2",
            "F3",
            "F4",
        ):
            raise ValueError("Stage 0 v1.1 bundle scope/family binding is invalid")
        assignments.append(
            {
                "scope": scope,
    ...
    result = {
        ...
        == len(assignments),
        "all_selected_gpus_fresh_idle": all(
            _idle(gpu) for gpu in idle[: len(assignments)]
        ),
    }
    result["pass"] = (
