"""Planner-only F4 corridor selection with fresh-scene hash fix v12."""
...
SCHEMA_VERSION = "cmf_f4_corridor_selection_gate_v12"


class F4CorridorSelectionGateV12:
    ...
    def _public_candidate(candidate, suffix, cleanup_pass):
        ...
        equivalence = suffix.get("evidence", {}).get(
    ...
    def run(self, *, output_dir: Path, planned_root_slot_spec) -> dict:
        ...
        try:
                ...
                def candidate_callback(scene, _program):
                    ...
                    try:
                        suffix = _validate_suffix_planner_receipt(
                            self.adapter.controller_v3_3.plan_a_exact_corridor_candidate_v12(
            ...
            hash_checks = {
                "at_least_one_candidate_reached_planner": bool(public)
                and sum(
                    int(item.get("planner_query_count", 0)) for item in public
                )
                > 0,
                "all_attempted_candidate_equivalence_pass": bool(public)
                and all(
                    item.get("fresh_scene_candidate_equivalence_pass") is True
                    for item in public
                ),
                "all_attempted_candidate_preplanner_contracts_exact": bool(public)
                and all(
            ...
            receipt["hash_infrastructure_audit_v12"] = {
                "schema_version": "cmf_f4_hash_infrastructure_audit_v12",
                "checks": hash_checks,
                "pass": all(hash_checks.values()),
            ...
            receipt["status"] = (
                "passed_f4_corridor_selection_gate_v12"
                if audit["pass"]
                else "failed_f4_corridor_selection_gate_v12"
            )
            ...
            receipt["status"] = "failed_f4_corridor_selection_gate_v12"
            receipt["error_type"] = type(exc).__name__
            receipt["error"] = str(exc)
            receipt["traceback"] = traceback.format_exc()
...
__all__ = ["F4CorridorSelectionGateV12"]
