"""Interface-fixed three-scene/no-suffix diagnostic for F3 V2_1."""
...
from .f3_common_grasp_prefix_v2 import PROGRAM_IDS, validate_f3_common_grasp_prefix_v2
from .f3_common_grasp_prefix_v2_1 import (
...
SCHEMA_VERSION = "cmf_f3_common_grasp_prefix_v2_1_diagnostic"
FINALIZER_SCHEMA_VERSION = "cmf_f3_common_grasp_prefix_v2_1_finalizer"


def _valid_context_binding(value: Any) -> bool:
    if not isinstance(value, Mapping):
        return False
    payload = dict(value)
    digest = payload.pop("context_binding_sha256", None)
    required = {
        "schema_version",
        "implementation_version",
        "binding_field",
    ...
    return (
        ...
        and digest == hash_json(payload)
        and payload["binding_sha256"] == hash_json(binding_payload)
        and isinstance(payload["prefix_contract_sha256"], str)
        and len(payload["prefix_contract_sha256"]) == 64
        and isinstance(payload["artifact_sha256"], str)
        and len(payload["artifact_sha256"]) == 64
    )


def finalize_f3_common_grasp_prefix_no_suffix_diagnostic_v2_1(
    ...
    *,
    ...
    prefix_hashes = {item.get("executed_prefix_action_sha256") for item in values}
    scene_ids = [item.get("scene_instance_id") for item in values]
    context_bindings = [item.get("repair_binding") for item in values]
    expected = dict(expected_binding)
    expected_digest = expected.get("binding_sha256")
    binding_core_pass = (
        isinstance(expected_digest, str)
        and expected_digest
        == hash_json({key: value for key, value in expected.items() if key != "binding_sha256"})
    )
    per_context_binding_pass = len(context_bindings) == 3 and all(
        _valid_context_binding(item) for item in context_bindings
    )
    same_context_binding = (
        len(context_bindings) == 3
        and context_bindings[0] == context_bindings[1] == context_bindings[2]
    )
    contexts_match_expected = per_context_binding_pass and all(
    ...
    checks = {
        ...
        "expected_binding_hash_valid": binding_core_pass,
        "all_context_binding_hashes_valid": per_context_binding_pass,
        "same_context_binding_all_scenes": same_context_binding,
        "all_contexts_match_constructor_binding": contexts_match_expected,
    }
    result = {
        "schema_version": FINALIZER_SCHEMA_VERSION,
        "implementation_version": IMPLEMENTATION_VERSION,
        "repair_binding": expected,
        "program_ids": ids,
        "scene_instance_ids": scene_ids,
        "prefix_action_sha256": next(iter(prefix_hashes)) if len(prefix_hashes) == 1 else None,
        "checks": checks,
...
class F3SharedPrefixNoSuffixDiagnosticV1_1:
    def __init__(self, adapter):
        if adapter.family != "F3":
            raise ValueError("F3 V2_1 prefix diagnostic requires family F3")
        controller = adapter.controller_v3_3
        if getattr(controller, "f3_shared_prefix_repair_v11", None) is not None:
            raise ValueError("F3 V2_1 diagnostic cannot coexist with v11")
        constructor_contract = getattr(
            controller, "f3_common_grasp_prefix_v2", None
        )
        if not isinstance(constructor_contract, Mapping):
            raise ValueError("F3 V2_1 constructor contract must be a mapping")
        self.repair = validate_f3_common_grasp_prefix_v2(constructor_contract)
        self.repair_binding = build_f3_common_grasp_prefix_binding_v2_1(self.repair)
    ...
    def _validate_prefix_contract(self, prefix_contract):
        return validate_bound_f3_common_grasp_prefix_v2_1(
            prefix_contract, expected_contract=self.repair
        )
    ...
    def run(self, *, output_dir: Path, planned_root_slot_spec) -> dict[str, Any]:
        ...
        _write_json(output_dir / "repair_binding.json", self.repair_binding)
        try:
            ...
            def reference_callback(scene, _program):
                ...
                prefix_contract = _immutable_copy(self.adapter.canonical_prefix_contract(programs))
                self._validate_prefix_contract(prefix_contract)
                before = int(getattr(scene, "planner_query_count", 0))
                receipt["execution_attempt_count"] += 1
                try:
                    prefix = _validate_prefix_reference_result(
                        self.adapter.plan_and_execute_canonical_prefix(scene, prefix_contract)
                    )
                    if prefix.get("prefix_physical_acceptance", {}).get("pass") is not True:
                        raise RuntimeError("F3 V2_1 reference prefix physical Gate failed")
                    prefix["trace_source"] = self._trace(
            ...
            try:
                current, anchor, programs, prefix_contract, prefix = self.helper._scene_call(
                    ...
                    phase="f3_v2_1_prefix_no_suffix_reference",
                    program=None,
                    program_sha256=None,
            ...
            reference_validation = self._validate_prefix_contract(manifest["prefix_contract"])
            context_binding = build_f3_common_grasp_prefix_context_binding_v2_1(
                reference_validation, artifact_sha256=manifest["artifact_sha256"]
            )
            reference_context = {
                ...
                "repair_binding": context_binding,
                "prefix_physical_acceptance": prefix["prefix_physical_acceptance"],
            ...
            for program in programs[1:]:
                ...
                def replay_callback(scene, _program):
                    replay_validation = self._validate_prefix_contract(manifest["prefix_contract"])
                    replay_binding = build_f3_common_grasp_prefix_context_binding_v2_1(
                        replay_validation, artifact_sha256=manifest["artifact_sha256"]
                    )
                    ...
                    if anchor_equivalence["equivalent"] is not True:
                        raise ValueError("F3 V2_1 no-suffix replay anchor mismatch")
                    ...
                    try:
                        ...
                        if physical.get("pass") is not True:
                            raise RuntimeError("F3 V2_1 no-suffix replay physical Gate failed")
                        return {
                            "program_id": program_id,
                            "execution_mode": "exact_replay",
                            "diagnostic_nonroot": True,
                            "pass": True,
                            "executed_prefix_action_sha256": replay["executed_prefix_action_sha256"],
                            "repair_binding": replay_binding,
                ...
                context = self.helper._scene_call(
                    ...
                    planned_spec=planned,
                    planned_spec_sha256=planned_hash,
                    phase=f"f3_v2_1_prefix_no_suffix_replay:{program_id}",
                    program=program,
                    program_sha256=hash_json(program),
                    callback=replay_callback,
                )
                context["scene_instance_id"] = receipt["cleanup_records"][-1]["scene_instance_id"]
                _write_json(context_dir / "receipt.json", context)
                receipt["contexts"].append(context)

            finalizer = finalize_f3_common_grasp_prefix_no_suffix_diagnostic_v2_1(
                receipt["contexts"],
                cleanup_records=receipt["cleanup_records"],
                expected_binding=self.repair_binding,
            )
            receipt["finalizer"] = finalizer
            receipt["pass"] = finalizer["pass"]
            receipt["status"] = (
                "passed_f3_common_grasp_prefix_v2_1_diagnostic"
                if finalizer["pass"]
                else "failed_f3_common_grasp_prefix_v2_1_diagnostic"
            )
            ...
            receipt["status"] = "failed_f3_common_grasp_prefix_v2_1_diagnostic"
            receipt["error_type"] = type(exc).__name__
            receipt["error"] = str(exc)
            receipt["traceback"] = traceback.format_exc()
