"""CPU-only immutable scope bundle builder for runtime-v3_4."""
...
from .probes.runtime_v3_4_authorization_v1 import (
...
from .runtime_v3_4_budget_v1 import SCOPE_FAMILIES, budget_receipt_sha256, scope_budget
from .runtime_v3_4_scope_specs_v1 import planned_scope_spec


VAULT_ROOT = Path("/nfs_share/lijunhui/Vault-on-Fvl09")
AUDIT_ROOT = VAULT_ROOT / "数据构造/实现审计"
PARENT_AUTHORIZATION = (
    AUDIT_ROOT / "USER_AUTHORIZATION_RUNTIME_V3_4_DIAGNOSIS_FIRST_20260830.json"
)
...
def _validate_reviewed_publication(commit: str) -> dict[str, Any]:
    ...
    active_sha = current_source_bindings_v3_4()["implementation_source_sha256"]
...
def load_parent_user_authorization(path: Path = PARENT_AUTHORIZATION) -> dict:
    ...
    if not isinstance(digest, str) or canonical_sha256(payload) != digest:
        raise ValueError("runtime-v3_4 parent authorization hash mismatch")
    if value.get("approved") is not True or value.get("stage0_authorized") is not False:
        raise ValueError("runtime-v3_4 parent authorization flags are invalid")
    return value
...
def build_scope_bundle(
    *,
    ...
    family = SCOPE_FAMILIES[scope]
    group = "runtime_v3_4_diagnosis_first_v1"
    ...
    child_command = (
        ...
        "-m",
        "controlled_multi_future.probes.runtime_v3_4_scope_runner",
    ...
    return {
        "schema_version": "cmf_runtime_v3_4_scope_bundle_v1",
        "scope": scope,
        "family": family,
