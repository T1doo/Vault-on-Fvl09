"""Fail-closed single-use authorization for one F1 batch-pilot family job."""
...
from ..f1_batch_pilot_scope_v1 import *
...
AUTH_SCHEMA = "cmf_f1_batch_pilot_authorization_v1"
CONSUMPTION_SCHEMA = "cmf_f1_batch_pilot_consumption_v1"
...
def validate(
    ...
    *,
    ...
    if expected_family not in (None, "F1"):
        raise AuthorizationBindingError("family mismatch")
    if expected_seed not in (None, 2026083101):
        raise AuthorizationBindingError("seed mismatch")
    ...
    frozen = budget()
    for key, expected in (
        ("budget_receipt_sha256", frozen["budget_receipt_sha256"]),
        ("planner_query_limit", frozen["planner_query_limit"]),
        ("controlled_action_limit", frozen["trajectory_execution_limit"]),
    ...
    for field, path, expected in (
        ...
        if (
            actual != path.resolve()
            or _fsha(actual) != result.get(field.replace("_path", "_file_sha256"))
            or json.loads(actual.read_text(encoding="utf-8")) != expected
        ):
            raise AuthorizationBindingError(f"publication {field}")
    frozen_spec = spec()
    if result.get("planned_root_slot_spec") != frozen_spec:
        raise AuthorizationBindingError("spec")
    if result.get("planned_root_slot_spec_sha256") != frozen_spec["planned_scope_spec_sha256"]:
        raise AuthorizationBindingError("spec hash")
    evidence = _file(result.get("source_evidence_path"), "evidence")
    if evidence != EVIDENCE.resolve() or _fsha(evidence) != result.get("source_evidence_file_sha256"):
        raise AuthorizationBindingError("evidence")
    source_path = _file(result.get("source_lock_receipt_path"), "source")
    source = load_runtime_source_lock(source_path, expected_family="F1")
    if (
