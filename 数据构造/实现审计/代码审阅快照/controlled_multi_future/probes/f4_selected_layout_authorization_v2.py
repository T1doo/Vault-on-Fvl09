"""Fail-closed authorization binding for the selected F4 V2 layout."""
...
from ..f4_selected_layout_scope_v2 import *
...
AUTH_SCHEMA = "cmf_f4_selected_layout_v2_authorization"
CONSUMPTION_SCHEMA = "cmf_f4_selected_layout_v2_consumption"
...
def validate(
    ...
    *,
    ...
    fixed = {
        ...
        "selected_layout_candidate_id": frozen_spec[
            "selected_layout_candidate_id"
        ],
        "selected_layout_candidate_sha256": frozen_spec[
            "selected_layout_candidate_sha256"
        ],
        "f4_layout_search_contract_sha256": frozen_spec[
            "f4_layout_search_contract_sha256"
        ],
        "f4_layout_dispatch_contract_sha256": frozen_spec[
            "f4_layout_dispatch_contract_sha256"
        ],
    }
    for key, expected in fixed.items():
        if result.get(key) != expected:
            raise AuthorizationBindingError(f"field changed {key}")
    if expected_family not in (None, "F4") or expected_seed not in (None, SEED):
    ...
    for key, expected in (
        ...
        (
            ...
            frozen_budget["canonical_prefix_reference_execution_limit"],
        ),
    ...
    for field, path, expected in (
        ("budget_publication_path", BUDGET, frozen_budget),
    ...
    if result.get("planned_root_slot_spec_sha256") != frozen_spec[
    ...
    if (
        ...
        or json.loads(evidence.read_text(encoding="utf-8")).get(
...
def consume(authorization, *, ledger_directory):
    ...
    value = {
        ...
        "family": "F4",
        "scene_seed": SEED,
        "selected_layout_candidate_id": spec()["selected_layout_candidate_id"],
