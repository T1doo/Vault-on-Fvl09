...
from ..runtime_v3_1_budget_v1_2 import (
    SUPPORTED_SCOPES,
...
AUTHORIZATION_SCHEMA_VERSION = "cmf_runtime_v3_1_gpu_authorization_v1_2"
CONSUMPTION_SCHEMA_VERSION = "cmf_runtime_v3_1_authorization_consumption_v1_2"
...
def implementation_source_sha256_v5_1() -> str:
...
def current_source_bindings_v1_2() -> dict:
    ...
    paths = {
        ...
        "gpu_guard_sha256": root / "probes/gpu_guard_v2_2.py",
        "authorization_validator_sha256": root / "probes/runtime_v3_1_authorization_v1_2.py",
        "runtime_source_lock_module_sha256": root / "runtime_source_lock_v1.py",
    }
    result = {key: sha256_file(path) for key, path in paths.items()}
    result["implementation_source_sha256"] = implementation_source_sha256_v5_1()
...
def validate_authorization_v1_2(
    ...
    *,
    ...
    bindings = current_source_bindings_v1_2()
...
def load_authorization_v1_2(path: Path, **kwargs) -> dict:
    path = Path(path)
    if not path.is_file():
        raise AuthorizationBindingError("runtime-v3_1 requires an explicit v1_2 authorization receipt")
    try:
        value = json.loads(path.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise AuthorizationBindingError("authorization receipt is unreadable") from exc
    return validate_authorization_v1_2(value, **kwargs)
