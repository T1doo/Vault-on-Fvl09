...
def run_f2(job, output, source_sha):
    ...
    from controlled_multi_future.f2_runtime_geometry_v5 import (
    ...
    for index, frozen in enumerate(job["candidates"], start=1):
        ...
        with opened_scene(
            ...
            try:
                runtime_geometry = capture_f2_runtime_geometry_observation_v5(
                    scene
                )
                runtime_geometry_gate = compare_f2_runtime_geometry_v5(
                    stage_a_spec["geometry_certificate"], runtime_geometry
                )
        ...
        write_new(
            candidate_dir / "stage_a_terminal.json",
            {
                ...
                "runtime_geometry_gate": runtime_geometry_gate,
            },
        )
...
def prepare_f3_scene(scene, adapter, recipe, expected_binding):
    from controlled_multi_future.f3_scene_binding_equivalence_v1_1 import (
    ...
    receipt = audit_f3_scene_binding_equivalence_v1_1(
        recipe=recipe,
...
def run_f4_full_program_gate(job, output, source_sha):
    ...
    from controlled_multi_future.root_orchestrator_v1_1 import (
        compare_three_branch_final_state_payloads,
    )

    prerequisite_path = Path(job["isolation_gate_terminal_path"])
    if file_sha(prerequisite_path) != job["isolation_gate_terminal_file_sha256"]:
        raise RuntimeError("F4 isolation Gate terminal file hash mismatch")
    prerequisite = json.loads(prerequisite_path.read_text(encoding="utf-8"))
    prerequisite_payload = dict(prerequisite)
    prerequisite_digest = prerequisite_payload.pop("receipt_sha256", None)
    prerequisite_self_hash_valid = (
        prerequisite_digest == canonical_hash(prerequisite_payload)
    )
    if (
        prerequisite_digest != job["isolation_gate_receipt_sha256"]
        or prerequisite.get("F4", {}).get("all_five_isolation_stages_pass")
        is not True
        or prerequisite.get("F4", {}).get("next_required_gate")
        != "ABC_ACB_BAC_real_physical_programs"
    ):
        raise RuntimeError("F4 full-program Gate lacks exact passing isolation evidence")
    ...
    for program_id in PROGRAM_IDS:
        ...
        if file_sha(path) != record["file_sha256"]:
            raise RuntimeError(f"F4 {program_id} source planner file hash mismatch")
        envelope = json.loads(path.read_text(encoding="utf-8"))
        terminal = envelope.get("terminal")
        prior_spec = envelope.get("spec")
        if not isinstance(terminal, dict) or not isinstance(prior_spec, dict):
            raise RuntimeError(f"F4 {program_id} source planner evidence is incomplete")
        spec = build_f4_full_program_physical_spec_v1(
            source,
            candidate,
            terminal,
            program_id=program_id,
            ...
            isolation_gate_receipt_sha256=prerequisite_digest,
        )
        source_records[program_id] = {
            "path": str(path),
            "file_sha256": record["file_sha256"],
            "terminal_receipt_sha256": terminal["receipt_sha256"],
            "planner_pass": terminal.get(
                "robot_kinematic_table_world_planner_pass"
            )
            is True,
        }
        specs[program_id] = spec
    if not all(item["planner_pass"] for item in source_records.values()):
        raise RuntimeError("F4 full-program source planner prerequisite changed")
    ...
    for index, program_id in enumerate(PROGRAM_IDS, start=1):
        ...
        program_dir = output / "physical" / f"{index:02d}_{program_id}"
        write_new(program_dir / "physical_spec.json", spec)
        adapter = adapter_for(
            "F4", spec["legacy_scene_spec"], program_dir / "scene", source_sha
        )
        scene_receipt = record_physical_scene(
            family="F4",
            adapter=adapter,
            legacy_scene_spec=spec["legacy_scene_spec"],
            output=program_dir / "execution",
            trace_actor_name="common_x",
            arm=spec["f4_source_grasp_candidate_v1"]["arm"],
            execute=lambda scene, spec=spec, adapter=adapter: run_f4_full_program_physical_v1(
                scene, spec, capture_anchor_callback=adapter.capture_anchor
            ),
            phase="F4_FULL_PROGRAM_PHYSICAL_GATE",
            program=program_id,
        )
        physical_count += 1
        terminal = scene_receipt.get("result") or {}
        passed = terminal.get("full_program_physically_qualified") is True
        if passed:
            success_count += 1
        current_hashes.append(scene_receipt["current"]["aggregate_sha256"])
        anchor_hashes.append(scene_receipt["anchor"]["anchor_sha256"])
        physical = terminal.get("physical_result", {})
        branch_payloads.append(
            {
                "program_id": program_id,
                "final_state_equivalence_payload": physical.get(
        ...
        rows.append(
            {
                ...
                "physical_attempted": True,
                "physical_pass": passed,
                "planner_query_count": terminal.get("planner_query_count"),
                "current_sha256": current_hashes[-1],
                "anchor_sha256": anchor_hashes[-1],
                "scene_receipt_sha256": scene_receipt["receipt_sha256"],
                "failure_category": None
                if passed
                else physical.get("earliest_failure")
                or (scene_receipt.get("error") or {}).get("type")
                or "F4_FULL_PROGRAM_GATE_FAILED",
            }
        )
        if not passed:
            break

    all_programs_executed = physical_count == len(PROGRAM_IDS)
    current_equivalent = (
        all_programs_executed and len(set(current_hashes)) == 1
    )
    anchor_equivalent = all_programs_executed and len(set(anchor_hashes)) == 1
    final_equivalence = (
        compare_three_branch_final_state_payloads(branch_payloads)
        if all_programs_executed
        else {
            "equivalent": False,
            "reason": "not_all_three_full_programs_executed",
            "pairwise": [],
        }
    )
    full_pass = (
        success_count == len(PROGRAM_IDS)
        and current_equivalent
        and anchor_equivalent
        and final_equivalence.get("equivalent") is True
    )
    return {
        "family": "F4",
        "mode": "ABC_ACB_BAC_REAL_PHYSICAL_PROGRAM_GATE_V1",
        "isolation_gate_receipt_sha256": prerequisite_digest,
        "isolation_gate_historical_self_hash_valid": prerequisite_self_hash_valid,
        "isolation_gate_exact_file_hash_binding_used": True,
        "source_planner_terminals": source_records,
        "planner_gate_pass": True,
        "rows": rows,
        "physical_execution_count": physical_count,
        "physical_success_count": success_count,
        "same_current_sha256": current_hashes[0]
        if current_equivalent
        else None,
        "same_anchor_sha256": anchor_hashes[0]
        if anchor_equivalent
        else None,
        "same_current_pass": current_equivalent,
        "same_anchor_pass": anchor_equivalent,
        "final_state_equivalence": final_equivalence,
        "physical_isolation_gate_pass": True,
        "full_program_pass": full_pass,
        "template_qualification_pass": full_pass,
        "development_root_pass": False,
        "accepted_trajectory_count": 0,
        "next_required_gate": "ONE_F4_DEVELOPMENT_R_PC_ROOT"
        if full_pass
        else "STOP_AND_REVIEW_FAILED_FULL_PROGRAM",
    }


def run_f4_development_r_pc_root(job, output, source_sha):
    from controlled_multi_future.f4_full_program_physical_v1 import (
        PROGRAM_IDS,
        build_f4_full_program_physical_spec_v1,
    )
    ...
    from controlled_multi_future.real_sapien_adapter_f4_qualified_root_v1 import (
    ...
    if file_sha(template_path) != job["template_gate_terminal_file_sha256"]:
        raise RuntimeError("F4 development root template terminal file changed")
    ...
    if (
        ...
        != "F4_FULL_PROGRAM_TEMPLATE_QUALIFICATION_PASS"
        or template.get("terminal_matrix", {}).get("full_program_pass")
        is not True
        or template.get("terminal_matrix", {}).get("development_root_pass")
        is not False
    ):
        raise RuntimeError("F4 development root lacks exact template qualification")
    ...
    for program_id in PROGRAM_IDS:
        ...
        path = Path(record["path"])
        if file_sha(path) != record["file_sha256"]:
            raise RuntimeError(f"F4 root {program_id} planner source changed")
        envelope = json.loads(path.read_text(encoding="utf-8"))
        prior_spec = envelope.get("spec")
        terminal = envelope.get("terminal")
        if not isinstance(prior_spec, dict) or not isinstance(terminal, dict):
            raise RuntimeError(f"F4 root {program_id} planner source is incomplete")
        full_specs[program_id] = build_f4_full_program_physical_spec_v1(
            ...
            slot_id=prior_spec["slot_id"],
            planner_reset_nonce=prior_spec["planner_reset_nonce"],
            isolation_gate_receipt_sha256=job[
                "isolation_gate_receipt_sha256"
            ],
        )
        planner_sources[program_id] = {
            "path": str(path),
            "file_sha256": record["file_sha256"],
            "receipt_sha256": terminal["receipt_sha256"],
        }
    planned_spec = full_specs["F4-ABC"]["legacy_scene_spec"]
    if any(
        spec["legacy_scene_spec_sha256"]
        != planned_spec["planned_scope_spec_sha256"]
        for spec in full_specs.values()
    ):
        raise RuntimeError("F4 root program specs do not share one planned scene")
    adapter = RoboTwinRealSapienF4QualifiedDevelopmentRootV1Adapter(
        output_root=output / "scene_work",
        expected_implementation_source_sha256=source_sha,
        planned_spec=planned_spec,
        full_program_specs=full_specs,
    )
    realization = {
        program_id: {
            "realization": "r_pc",
    ...
    root = RealSapienStrictPrefixRootOrchestratorV1_2(
        ...
        output_dir=output / "development_root",
    ...
    return {
        "family": "F4",
        "mode": "ONE_F4_DEVELOPMENT_R_PC_ROOT_V1",
        "template_gate_receipt_sha256": template_digest,
        "source_planner_terminals": planner_sources,
        "planner_gate_pass": root.get("family_suffix_gate", {}).get("pass")
        is True,
        "physical_execution_count": int(
            root.get("branch_execution_attempt_count", 0)
        ),
        "physical_success_count": branch_successes,
        "physical_isolation_gate_pass": True,
        "template_qualification_pass": True,
        "full_program_pass": accepted,
        ...
        "accepted_trajectory_count": 0,
        "root_receipt_relative_path": "development_root/root_receipt.json",
        "root_receipt": root,
        "next_required_gate": "F4_R_INV_PATH_AND_R_INV_MOTION_IMPLEMENTATION"
        if accepted
        else "STOP_AND_REVIEW_F4_DEVELOPMENT_ROOT_FAILURE",
    }
...
def main(argv=None):
    ...
    try:
        if job["family"] == "F2":
            ...
            mode = job.get("mode")
            if mode == "ABC_ACB_BAC_REAL_PHYSICAL_PROGRAM_GATE_V1":
                result = run_f4_full_program_gate(job, output, live_source)
            elif mode == "ONE_F4_DEVELOPMENT_R_PC_ROOT_V1":
                result = run_f4_development_r_pc_root(job, output, live_source)
            else:
