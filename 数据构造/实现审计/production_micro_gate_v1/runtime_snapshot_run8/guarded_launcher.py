...
def load_manifest(path: Path):
    ...
    if value.get("source_freeze_vault_head") != "0596fddc8add58a75f86e93f67c61c61a3fcc578":
...
def main(argv=None):
    ...
    post_selected = None
    returned = False
    post_release_poll_snapshots = []
    if pre is not None and post is not None:
        pre_selected = selected(pre, args.physical_index, args.expected_uuid)
        current_post = post
        for poll_index in range(13):
            post_selected = selected(
                current_post, args.physical_index, args.expected_uuid
            )
            post_release_poll_snapshots.append(current_post)
            returned = bool(
                post_selected["memory_used_mib"]
                <= max(64, pre_selected["memory_used_mib"] + 32)
                and post_selected["utilization_gpu_percent"] == 0
                and post_selected["pstate"] in {"P8", "P12"}
                and not post_selected["compute_processes"]
            )
            if returned or poll_index == 12:
                break
            time.sleep(5)
            try:
                current_post = nvidia_snapshot()
            except BaseException as exc:
                cleanup_errors.append(
                    {
                        "post_release_poll_error": type(exc).__name__,
                        "message": str(exc),
                    }
                )
                break
    ...
    terminal = {
        ...
        "post_snapshot": post,
        "post_release_poll_snapshots": post_release_poll_snapshots,
