...
def finalize_cohort(cohort,directory,branches):
    ...
    from realization_utf8_publication_v1 import publish_final_branch,register_completed_root
...
def run_batch(manifest,*,collector=collect_cell):
    ...
    try:
        for cohort in catalog['cohorts']:
            ...
            if cohort['cohort']=='F1_A_path':
                recovered=read(manifest['recovered_branch_path'])
                write_new(directory/'branches'/'F1-red'/'receipt.provisional.json',recovered)
                own.append(recovered)
            for cell in cohort['cells']:
                if cell['cell_id']=='F1_A_path__F1-red':continue
                # Exclusive durable start also prevents accidental cell reissue.
                replacement=cell['cell_id']=='F1_A_path__F1-red'
                write_new(directory/'attempts'/(cell['cell_id']+'.start.json'),seal({'cell':cell,'attempt_number':1,
                    'logical_cell_attempt_number':2 if replacement else 1,
                    'attempt_kind':'explicit_user_authorized_replacement' if replacement else 'previously_unattempted_cell',
                    'recovery_authorization_sha256':manifest['recovery_authorization_sha256'],'manifest_sha256':manifest['manifest_sha256']}))
                shared=Path(manifest['retained_F1_A_current_directory']) if cohort['cohort']=='F1_A_path' else directory/'current'
                branch=collector(cell,directory/'branches'/cell['program']['program_id'],shared_current_dir=shared)
    ...
    passed=not global_stop and error is None and len(branches)==8 and len(cohorts)==3 and all(c['status']=='accepted' for c in cohorts)
    terminal=seal({'schema_version':'cmf_realization_batch_terminal_v1','manifest_sha256':manifest['manifest_sha256'],'pass':passed,
        ...
        'previous_consumed_scene_count':2,'cumulative_scene_attempts':2+len(starts),'cumulative_scene_cap':10,
        'recovery_authorization_sha256':manifest['recovery_authorization_sha256'],
        'retained_prior_raw_trajectory_count':1,'prior_reconciled_planner_queries':11,'cumulative_planner_queries':None if count is None else 11+count,
        'reused_prior_cell_ids':['F1_A_path__F1-red'],
