...
class RealArtifactTests(unittest.TestCase):
    ...
    def test_actual_cohort_two_phase_publication(self):
        ...
        self.assertEqual(Path(sys.modules['job_runner'].__file__).resolve().parent,Path(__file__).resolve().parent)
    ...
    def test_parent_source_binding_before_scene(self):
        for group in self.catalog['cohorts']:
            cell=group['cells'][0];adapter=make_adapter(cell,W/'Robotwin2/tmp/cmf_unused_source_check')
            ref=json.loads((Path(cell['parent_root'])/'reference_current_hashes.json').read_text())
            self.assertEqual(adapter._sealed_implementation_source_sha256,ref['reconstruction_spec_audit']['simulation_configuration']['implementation_source_sha256'])

    def test_mismatch_retains_candidate_and_receipt(self):
        from unittest.mock import patch
        from pipeline import collect_cell
        cell=self.catalog['cohorts'][2]['cells'][0]
        candidate=json.loads((Path(cell['parent_root'])/'reference_current_hashes.json').read_text())
        candidate['reconstruction_spec_aggregate_sha256']='0'*64
        scene=SimpleNamespace(planner_query_count=0)
        class Context:
            cleanup_receipt={'cleanup_safety_pass':True,'orphan_process_count':0,'scene_instance_id':'cpu_mismatch'}
            def __enter__(self):return SimpleNamespace(scene=scene)
            def __exit__(self,*exc):return False
        adapter=SimpleNamespace(scene=lambda *a,**k:Context(),capture_current=lambda s:candidate)
        with tempfile.TemporaryDirectory(dir=W/'Robotwin2/tmp',prefix='cmf_cpu_mismatch_') as tmp,patch('pipeline.make_adapter',return_value=adapter):
            b=collect_cell(cell,Path(tmp)/'cell',shared_current_dir=Path(tmp)/'current')
            self.assertEqual(b['error']['type'],'SameCurrentMismatch');self.assertTrue(b['global_stop'])
            self.assertEqual(b['branch_current'],candidate)
            self.assertEqual(b['same_current_mismatch_receipt']['candidate_hashes']['reconstruction_spec_aggregate_sha256'],'0'*64)
            self.assertEqual(json.loads((Path(tmp)/'cell/candidate_current.json').read_text()),candidate)

    def test_old_execution_authorization_rejected(self):
        from manifest_contract import load_manifest
        with self.assertRaisesRegex(PermissionError,'exact replacement/resume approval required'):
            load_manifest(W/'Vault-on-Fvl09/数据构造/实现审计/REALIZATION_BATCH_APPROVED_MANIFEST_V1_20260905.json')

    def test_cumulative_recovery_counter_and_unknown_fail_closed(self):
        from manifest_contract import validate_terminal
        t={'previous_consumed_scene_count':2,'cumulative_scene_attempts':3,'cumulative_scene_cap':10,
            'fresh_scene_attempts':1,'raw_trajectory_count':0,'accounting_complete':False,'planner_queries':None,'global_stop':True,
            'pass':False,'new_independent_root_count':0,'stage1_accepted_trajectory_count':0,'formal_trajectory_count':0,
            'error':{'type':'CPU injected missing counter'},'cells':[],'cohorts':[]}
        self.assertFalse(validate_terminal(t))
        for bad in ({**t,'cumulative_scene_attempts':1},{**t,'planner_queries':0},{**t,'global_stop':False}):
            with self.assertRaises(ValueError):validate_terminal(bad)

    def test_utf8_publication_forced_ascii_locale(self):
        from realization_utf8_io_v1 import cpu_test
        self.assertTrue(cpu_test()['forced_C_locale_utf8_roundtrip'])

    def test_full_eight_cell_dispatch_reuses_first_without_execution(self):
        from job_runner import run_batch
        from pipeline import write_new
        from manifest_contract import validate_terminal
        calls=[]
        def fixture_collector(cell,output,*,shared_current_dir):
            calls.append(cell['cell_id']);b=source_branch(cell)
            original=json.loads((Path(cell['parent_root'])/'root_receipt.json').read_text())
            b['cleanup']=next(r for r in original['cleanup_records'] if cell['program']['program_id'] in r['scene_instance_id'] and 'strict_prefix_branch' in r['scene_instance_id'])
            b.update(cell_id=cell['cell_id'],planner_query_delta=cell['query_cap'],accounting_complete=True,global_stop=False,raw_written=True,cpu_fixture=True)
            write_new(Path(output)/'receipt.provisional.json',b);return b
        with tempfile.TemporaryDirectory(dir=W/'Robotwin2/tmp',prefix='cmf_cpu_continuation_') as tmp:
            manifest={'catalog_path':str(W/'Vault-on-Fvl09/数据构造/实现审计/REALIZATION_BATCH_CATALOG_V1_20260905.json'),
                'jobs':[{'output_namespace':str(Path(tmp)/'batch')}],'manifest_sha256':'cpu_fixture','recovery_authorization_sha256':'cpu_fixture',
                'recovered_branch_path':str(W/'Robotwin2/datasets/cmf_realization_utf8_receipt_recovery_v1/F1-red.derived_v1_1.json'),
                'retained_F1_A_current_directory':str(W/'Robotwin2/datasets/cmf_realization_recovery_v1_2/F1_A_path/current')}
            result=run_batch(manifest,collector=fixture_collector)
            self.assertTrue(result['pass'],result.get('error'));self.assertTrue(validate_terminal(result))
            self.assertEqual(len(calls),8);self.assertNotIn('F1_A_path__F1-red',calls)
            self.assertEqual(result['planner_queries'],112);self.assertEqual(result['cumulative_scene_attempts'],10)
