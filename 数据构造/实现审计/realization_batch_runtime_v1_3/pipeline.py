...
from realization_utf8_io_v1 import write_new as atomic_utf8_write

class ScientificFailure(RuntimeError):pass
def write_new(path,value):
    return atomic_utf8_write(path,value)
...
def collect_cell(cell,output,*,shared_current_dir):
    ...
    started=time.monotonic();branch={'program_id':program['program_id'],'cell_id':cell['cell_id'],'realization':cell['variant'],'root_id':cell['parent_root_id'],
        'candidate_universe_sha256':cell['candidate_universe_sha256'],'prefix_sha256':prefix['artifact_sha256'],'prefix_artifact_sha256':prefix['artifact_sha256'],
        'prefix_contract_sha256':prefix['prefix_contract_sha256'],'reference_current_sha256':reference['aggregate_sha256'],
        ...
        with context as handle:
            ...
            try:
                ...
                result['provenance'].update(trace_source_sha256=sha(trace),trace_source_relative_path='../trace_source.npz',development_data=True,
                    implementation_version='cmf_development_realization_batch_v1_3',parent_root=str(parent),parent_source_branch_sha256=cell['source_branch_file_sha256'],
                ...
                branch.update(raw_manifest=raw,raw_directory=str(output/'raw'),verifier=verifier,realized_variation=variation,final_state_equivalence_payload=result.get('final_state_equivalence_payload'))
