...
def run_candidate(scene, recipe, stage_a_terminal, *, planner_seed):
    ...
    def lift_action():
        baseline["lift_receipt"]=_execute_planned_segment(scene,planned["controls"],targets,2,arm)
        _wait_and_record(scene,50)
    def verify():
        from post_lift_audit import audit_micro_lift_trace
        return audit_micro_lift_trace(scene.trace,baseline_row=baseline["row"],lift_receipt=baseline["lift_receipt"],
          arm=arm,selected_links=list(scene.selected_gripper_links()),bottle_name=_entity(scene.bottle).get_name(),
