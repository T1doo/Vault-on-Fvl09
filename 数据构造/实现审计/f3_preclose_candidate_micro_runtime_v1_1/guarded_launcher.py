#!/usr/bin/env python3
"""Independent GPU Guard for a future externally-approved F3 pre-close micro."""
...
def _load_base_guard_primitives():
    ...
    spec = importlib.util.spec_from_file_location(
        "cmf_f3_micro_v1_base_guard_primitives", BASE_GUARD
    )
...
def main(argv=None) -> int:
    ...
    try:
        ...
        start_receipt = {
            "schema_version": "cmf_f3_preclose_micro_v1_guard_start_v1",
        ...
        if cache_job.exists():
            raise FileExistsError("F3 micro V1 cache job must be new")
        ...
        child_environment["CMF_F3_GUARD_START_RECEIPT"] = str(start_path)
        launch = BASE.nvidia_snapshot()
        if not BASE.idle(BASE.selected(launch, args.physical_index, args.expected_uuid)):
            raise RuntimeError("GPU stopped being idle before F3 child launch")
    ...
    terminal = {
        "schema_version": "cmf_f3_preclose_micro_v1_guard_terminal_v1",
        "run_id": manifest["run_id"],
        "job_id": args.job_id,
        "family": "F3",
        "manifest_sha256": manifest["manifest_sha256"],
    ...
    post_receipt = {
        "schema_version": "cmf_f3_preclose_micro_v1_post_child_validation_v1",
        "manifest_sha256": manifest["manifest_sha256"],
        "job_id": args.job_id,
