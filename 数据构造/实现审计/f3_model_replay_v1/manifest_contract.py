...
def load_manifest(path,*,execution=True,runner=False,post=False):
    ...
    job=m['jobs'][0]
    for k,v in {'family':'F3','fresh_scene_cap':0,'trajectory_query_cap':0,'physical_attempt_cap':0,'states_per_asset':5,'timeout_seconds':3600}.items():
    ...
    if m['previous_non_action_scenes']!=2 or m['cumulative_non_action_scene_cap']!=2:raise PermissionError('remaining scene budget')
    if m['live_scene_recipe_id']!='none':raise PermissionError('only remaining live asset scene permitted')
    if runner:
