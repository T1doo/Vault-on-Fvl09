...
class Tests(unittest.TestCase):
    def case(self,fail_plan=None,fail_window=None):
        route=json.loads((Path(__file__).parent/'route_spec.json').read_text())
        proposal=json.loads((Path(__file__).parent.parent/'f3_contact_height_revision_v1/recipe.json').read_text())
        proposal['_route_pregrasp_pose']=route['intermediate_actual_pregrasp_pose'];calls=[]
        actor=SimpleNamespace(get_name=lambda:'bottle');scene=SimpleNamespace(robot=SimpleNamespace(left_planner=object()),planner_query_count=0,trace=[{}],bottle=actor,pad=actor,selected_gripper_links=lambda:['fl_link7','fl_link8'])
        ...
        with tempfile.TemporaryDirectory(dir='/nfs_share/lijunhui/Robotwin2/tmp') as d,ExitStack() as stack:
    ...
    def test_success_exactly_four_queries(self):
        s,c,r=self.case();self.assertEqual(s.planner_query_count,4);self.assertTrue(r['pass']);self.assertEqual([x for x in c if x.startswith('plan:')],['plan:route_pregrasp','plan:pregrasp','plan:grasp','plan:lift25'])
    def test_every_preclose_planner_failure_stops(self):
        for i,stage in enumerate(('route_pregrasp','pregrasp','grasp')):
    ...
    def test_lift_planner_failure_no_fifth_query(self):
        s,c,r=self.case(fail_plan='lift25');self.assertEqual(s.planner_query_count,4);self.assertFalse(r['pass'])
