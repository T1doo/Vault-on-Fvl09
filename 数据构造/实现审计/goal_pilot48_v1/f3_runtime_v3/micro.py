...
def execute(scene,proposal,out):
    ...
    targets['route_pregrasp']=np.array(proposal['_route_pregrasp_pose'])
    ...
    def plan_and_execute(stage,actual):
        ...
        if before>=4:raise ValueError('four trajectory query cap')
    ...
    for stage in ('route_pregrasp','pregrasp','grasp'):
...
def run(manifest):
    ...
    cleanup=ctx.cleanup_receipt;known=type(before)is int and type(after)is int and 0<=after-before<=4
