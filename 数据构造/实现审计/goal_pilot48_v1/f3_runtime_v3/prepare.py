"""One fixed pregrasp route and 11 native open-hand geometry samples."""
...
def main():
    height=json.loads((O.parent/'f3_contact_height_revision_v1/recipe.json').read_text())
    oldpath=D/'p48_f3_micro_002/pregrasp.plan.json';old=json.loads(oldpath.read_text())
    ...
    assert abs(target[2]-start[2]-.010)<1e-12
    route=seal({'schema_version':'p48_f3_pregrasp_via_successful_lower_pose_v1','height_recipe_id':height['proposal_id'],
        ...
        'source_successful_window_sha256':sha(D/'p48_f3_micro_002/pregrasp.full_window.json'),
        'max_single_queries':4,'new_scene_budget':1,'pregrasp_route_revision':1,'postlift_roll_recipe_revision':2,
        'fixed_stages':['route_pregrasp','pregrasp','grasp','lift25'],'no_retry_or_target_sweep':True})
    assert json.loads((D/'p48_f3_micro_002/pregrasp.full_window.json').read_text())['pass']
