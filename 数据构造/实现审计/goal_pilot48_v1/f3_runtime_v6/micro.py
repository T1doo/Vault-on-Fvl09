...
def attach_closed_bottle_model(scene,out):
    from goal_pilot48_v1.f3_one_sided_escape_v2.live import prepare
    return prepare(scene,out,_build_full_attached_bottle_model)

def _old_native_plan_gate(scene,controls,out):
...
def native_plan_gate(scene,controls,out):
    from goal_pilot48_v1.f3_one_sided_escape_v2.live import native_gate
    return native_gate(scene,controls,out,_old_native_plan_gate)

def execute(scene,proposal,out):
    from goal_pilot48_v1.f3_one_sided_escape_v2.live import restore
    scene._cmf_tangent_scene_id=scene._cmf_scene_instance_id
    if not isinstance(scene._cmf_tangent_scene_id,str) or not scene._cmf_tangent_scene_id:raise ValueError('actual fresh scene identity absent')
    result=None;primary=None
    try:
        result=_execute_inner(scene,proposal,out)
        return result
    except BaseException as exc:
        primary=exc
        raise
    finally:
        if hasattr(scene,'_cmf_escape_model_branch'):
            try:
                checks=restore(scene,out,_build_full_attached_bottle_model)
            except BaseException as restoration_error:
                write_new(Path(out)/'restoration_failure.json',{'type':type(restoration_error).__name__,'message':str(restoration_error),'primary_error_preserved':primary is not None})
                if primary is None:raise
                checks={}
            if result is not None:
                result['full_world_restoration_checks']=checks
                restored_valid=(isinstance(checks,dict) and set(checks)=={'motion_gen','motion_gen_batch'} and all(c.get('valid') is True for c in checks.values()))
                if result.get('pass') and not restored_valid:
                    result['pass']=False;result['earliest_failed_stage']='full_world_restoration_not_valid'

def _execute_inner(scene,proposal,out):
    ...
    targets['pregrasp']=np.array(proposal['_route_pregrasp_pose'])
    ...
    def plan_and_execute(stage,actual):
        ...
        if stage=='lift25':
            from goal_pilot48_v1.f3_one_sided_escape_v2.live import begin_plan
            begin_plan(scene,actual)
        ...
        actual_names,actual_q=runtime_joint_state(scene)
        native=check_controls(planned['controls'][0]['position'],list(scene.robot.left_planner.motion_gen.kinematics.joint_names),actual_names,actual_q)
        write_new(out/(stage+'.native_self_pair_gate.json'),native)
        if not native['pass']:
            planned['native_self_pair_preexecution_gate']=native
            return None,planned
        if stage=='lift25':
    ...
    for stage in ('pregrasp','grasp'):
        ...
        if receipt is None:return {'pass':False,'earliest_failed_stage':stage+'_native_self_pair_gate' if 'native_self_pair_preexecution_gate' in plan else stage+'_planner','events':events,'windows':windows,'plans':plans,'close_executed':False}
    ...
    if receipt is None:return {'pass':False,'earliest_failed_stage':'lift_native_self_pair_gate' if 'native_self_pair_preexecution_gate' in plan else 'lift_native_preexecution_gate' if 'native_preexecution_gate' in plan else 'actual_postclose_lift_planner','events':events,'windows':windows,'plans':plans,'close_executed':True,'post_close_baseline_row':baseline}
...
def run(manifest):
    ...
    route=json.loads(Path(manifest['route_spec_path']).read_text(encoding='utf-8'))
    import hashlib
    for source_path,expected_hash in route['sources'].items():
        if hashlib.sha256(Path(source_path).read_bytes()).hexdigest()!=expected_hash:raise ValueError('route/native source binding mismatch: '+source_path)
    if route['height_recipe_id']!=proposal['proposal_id']:raise ValueError('route recipe mismatch')
    proposal['_route_pregrasp_pose']=route['intermediate_actual_pregrasp_pose']
