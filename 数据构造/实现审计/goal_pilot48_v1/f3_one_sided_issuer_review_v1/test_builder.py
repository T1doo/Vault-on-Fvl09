...
class Tests(unittest.TestCase):
    def reservation(self,job='p48_f3_one_sided_micro_cpu_builder_probe'):
    ...
    def test_real_evidence_build_is_pure_and_keeps_every_parent_dependency(self):
        job='p48_f3_one_sided_micro_cpu_builder_probe';statepaths=[issuer.ROOT/n for n in ('STATE.json','budget_ledger.jsonl','attempts.jsonl')]
        ...
        parent=issuer.read(issuer.ROOT/'jobs/p48_f3_tangent_micro_001.json')
        for field in ('source_files','input_files'):
            for p,h in parent[field].items():self.assertEqual(m[field][p],h)
        self.assertEqual(m['jobs'][0]['test_module'],'goal_pilot48_v1.f3_runtime_v6.test_all')
    ...
    def test_missing_or_wrong_budget_rejected_before_dependencies(self):
        with patch.object(issuer,'prerequisites',side_effect=AssertionError('must reject first')):
            for reservation in ({},dict(self.reservation(),reserved={})): 
                with self.assertRaises(ValueError):issuer.build_manifest('p48_f3_one_sided_micro_cpu_builder_probe',reservation)
    ...
    def test_current_CPU_hash_mismatch_rejected(self):
        ...
        def read(p):
            d=real_read(p)
            if str(p).endswith('CPU_AUDIT_V2_1.json'):d['receipt_sha256']='f'*64
            return d
    ...
    def test_old_parent_source_hash_conflict_is_rejected(self):
        real_read=issuer.read
        def read(p):
            d=real_read(p)
            if str(p).endswith('/jobs/p48_f3_tangent_micro_001.json'):
                first=next(iter(d['source_files']));d['source_files'][first]='f'*64
                d.pop('manifest_sha256');d['manifest_sha256']=issuer.digest(d)
            return d
        with patch.object(issuer,'read',side_effect=read):
            with self.assertRaisesRegex(ValueError,'changed parent dependency'):issuer.prerequisites()
    def test_CPU_input_binding_hash_conflict_rejected(self):
        real_read=issuer.read
        def read(p):
            d=real_read(p)
            if str(p).endswith('/CPU_INPUT_BINDINGS.json'):
                first=next(iter(d['input_bindings']));d['input_bindings'][first]='f'*64
                d.pop('receipt_sha256');d['receipt_sha256']=issuer.digest(d)
            return d
        with patch.object(issuer,'read',side_effect=read):
            with self.assertRaisesRegex(ValueError,'changed real CPU input'):issuer.prerequisites()
