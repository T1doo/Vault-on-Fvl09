"""Independent 5-MG local-versus-live-meter reconciliation."""
import json
...
def reconcile(result,counts,events):
    ...
    for e in events:
        ...
        totals[key]+=n
        if totals[key]!=e['total']:raise ValueError('meter event cumulative total mismatch')
    ...
    valid=all(type(n) is int and 0<=n<=CAPS[k] for k,n in expected.items())
    valid=valid and expected==counts==totals and result.get('ik_problem_attempts')==0
...
def run(manifest,*,meter):
    if meter is None or any(meter.counts.values()):raise ValueError('fresh installed live meter required')
    ...
    value['accounting_complete']=bool(r['accounting_complete'] and audit['pass'])
    value['pass']=bool(r['pass'] and value['accounting_complete']);value['scientific_route_pass']=bool(r['scientific_route_pass'] and value['pass'])
    value['receipt_sha256']=digest(value)
    from realization_utf8_io_v1 import write_new
    write_new(out/'inside_meter_checked_terminal.json',value);return value
