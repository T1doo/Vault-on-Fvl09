...
from goal_pilot48_v1.f3_runtime_v5.test_all import MODULES
def main():
    ...
    paths=list(O.glob('*.py'))+list((G/'f3_runtime_v5').glob('*.py'))+[A/'support_pair_collision_v1/policy.py',A/'support_pair_collision_v1/factory.py',G/'runtime/support_witness.py',G/'f3_native_self_pair_v1/checker.py']
    r={'schema_version':'p48_f3_tangent_escape_CPU_implementation_audit_v1','pass':True,'tests_run':result.testsRun,'elapsed_seconds':time.monotonic()-start,
    ...
    r['supersedes_CPU_AUDIT_file_sha256']=hashlib.sha256((O/'CPU_AUDIT.json').read_bytes()).hexdigest()
    r['factory_and_actual_state_binding_hardening']=True
    r['receipt_sha256']=hashlib.sha256(json.dumps(r,sort_keys=True,separators=(',',':'),ensure_ascii=False).encode('utf-8')).hexdigest();write_new(O/'CPU_AUDIT_V1_1.json',r);print(r['receipt_sha256'])
