#!/usr/bin/env python3
"""Independent GPU Guard for a future externally-approved F3 non-action model conformance."""
...
from goal_pilot48_v1.runtime_v2.manifest_contract import (  # noqa: E402
    GUARD_ENTRY,
    POST_CHILD,
    canonical_hash,
    file_sha,
...
def main(argv=None) -> int:
    ...
    from goal_pilot48_v1.runtime_v2.manifest_contract import load_manifest
    if args.preflight_only:
        from goal_pilot48_v1.runtime_v2.job_runner import main as runner_main
    ...
    try:
        ...
        start_receipt = {
            "schema_version": "cmf_goal_job_v1_guard_start_v1",
        ...
        child_environment["CMF_MODEL_GUARD_START_RECEIPT"] = str(start_path)
        ...
        if cache_job.exists():
            ...
            cache_removed = True
        if child is None:
            # No child/worker ever launched: other users' busy baseline is not
            # ours to restore. Record it without polling for their job to stop.
            post_check = BASE.nvidia_snapshot()
            cooldown = {"snapshots": [post_check], "pass": True, "errors": [],
                        "reason": "no child launched; no task GPU allocation",
                        "baseline_restoration_applicable": False}
            returned = False
        else:
    ...
    cleanup_pass = not cleanup_errors and cache_removed and lease_released and (returned or child is None)
    terminal = {
        "schema_version": "cmf_goal_job_v1_guard_terminal_v1",
        "run_id": manifest["run_id"],
        "job_id": args.job_id,
        "family": job["family"],
        "manifest_sha256": manifest["manifest_sha256"],
        ...
        "job_launched": child is not None,
        "execution_budget_consumed": child is not None,
    ...
    post_receipt = {
        "schema_version": "cmf_goal_job_v1_post_child_validation_v1",
        "manifest_sha256": manifest["manifest_sha256"],
        "job_id": args.job_id,
