...
def hold_witness(rows,*,selected_links,bottle_name,pad_name):
    ...
    for row in rows:
        ...
        for p in row['contact_pairs']:
            ...
            if bottle_name in bodies:touched|=bodies&selected;supported|=pad_name in bodies
    ...
    return {'frames':250,'support_actor_name':pad_name,'all_selected_contact':contact,'all_both_fingers':both,'all_signal_complete':complete,'forbidden_count':forbidden,'support_contact_frames':support_count,'max_relative_translation_drift_m':max(td),'max_relative_orientation_drift_rad':max(rd)}
...
def bind_exact_pad(witness,export,pad_actor_name):
    matches=[s for s in export['shapes'] if s['name']=='pad__0']
    if len(matches)!=1 or matches[0]['actor_name']!=pad_actor_name or witness['hold']['support_actor_name']!=pad_actor_name:
        raise ValueError('support witness does not bind the actually excluded pad actor/shape')
    if witness['hold']['support_contact_frames']!=250:raise ValueError('table contact cannot replace pad support witness')
    witness['excluded_support_binding']={'shape_name':'pad__0','actor_name':pad_actor_name,'geometry_sha256':matches[0].get('geometry_sha256')}
    return witness
