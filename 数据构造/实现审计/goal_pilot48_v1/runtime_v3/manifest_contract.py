...
def load_manifest(path,*,execution=True,runner=False,post=False):
    from goal_pilot48_v1.runtime_v3.budget import snapshot,rows
    ...
    job=m['jobs'][0]
    if job.get('family')=='F1':raise PermissionError('runtime_v2 new F1 dense-action coverage not audited')
    if job['resource_caps']['collection_attempts']>0 and job.get('requires_live_meter') is not True:raise PermissionError('collection requires live meter injection')
    snap=snapshot()
