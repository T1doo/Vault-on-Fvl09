...
import numpy as np
SCHEMA='stable_grasp_one_sided_native_unloading_escape_v2'
def seal(value):
...
def validate(c):
    try:
        ...
        if old['hold']!=hold or old['pairs']!=pairs or old.get('only_attached_bottle_support_pairs_overlap') is not True or verify_support_witness(old):return False
        return bool(c['schema_version']==SCHEMA and c['old_supported_hold_pass'] is False and
            c['model_eligibility_revision']==2 and c['native_lower_bound_epsilon_m']==.0001 and c['positive_gap_upper_bound_m'] is None and
            ...
            g.shape==(250,) and np.isfinite(g).all() and (g>=-.0001).all() and
