...
def prepare(scene,out,build_full):
    ...
    if old_supported:
        ...
        certificate=seal({'schema_version':SCHEMA,'old_supported_hold_pass':old_supported,'old_supported_witness':witness,
            'model_eligibility_revision':2,'native_lower_bound_epsilon_m':.0001,'positive_gap_upper_bound_m':None,
            'stability_recipe_revision_unchanged':3,'pregrasp_route_revision_unchanged':2,
        ...
        write_new(out/'one_sided_certificate_candidate.json',certificate)
        if not validate(certificate):raise ValueError('new tangent certificate rejected; old supported condition remains failed')
        lease=SinglePlanLease(certificate);factory=tangent_factory;factory_kwargs={'lease':lease};branch='one_sided_unloading'
...
def native_gate(scene,controls,out,old_gate):
    ...
    write_new(out/'lift_native_one_sided_escape_gate.json',result);return result
