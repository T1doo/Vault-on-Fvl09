...
def publish(job_id):
    ...
    if not terminal['pass'] or not terminal['runtime_result']['render_pass']:
        raise ValueError('render not verified')
    late = None
    if not guard['task_owned_cleanup_pass'] or not guard['gpu_returned_to_idle_baseline']:
        late = checked(ROOT/'hd_late_release_review_v1/OBSERVATION_001.json')
        if (late['receipt_sha256'] != '9a4cdaa06c612a58d7cd73040db72c8302e0f230f1b97a8397771d9d1f3e65d4'
                or late['job_id'] != job_id or late['original_guard_receipt_sha256'] != guard['receipt_sha256']
                or not late['late_baseline_restoration_verified'] or not late['own_pid_and_group_absent']
                or late['remaining_owned_process_rows'] or late['foreign_processes_signalled']):
            raise ValueError('exact independent late release proof missing')
        for p,h in late['immutable_file_hashes'].items():
            if sha(p) != h: raise ValueError('late release source changed')
        events = [e for e in budget.rows() if e['kind']=='RECONCILE' and e['job_id']==job_id]
        if len(events)!=1 or events[0]['evidence'].get('late_observation_receipt_sha256')!=late['receipt_sha256']:
            raise ValueError('late release not reconciled')
    ...
    result={'schema_version':'reviewed_hd_display_publication_v2','job_id':job_id,'label':item['label'],
    ...
    result['late_release_observation_receipt_sha256']=late['receipt_sha256'] if late else None
    result['original_guard_cleanup_pass']=guard['task_owned_cleanup_pass']
    result['original_guard_modified']=False
