...
import numpy as np
from goal_pilot48_v1.f3_runtime_v3 import micro
...
def execute(scene,proposal,out):
    ...
    if initial_count!=4:raise ValueError('fresh V3 micro must consume exactly four query problems')
    ...
    def plan_actual(s,targets,**kwargs):
        ...
        if scene.planner_query_count+len(targets)>15:raise ValueError('total fifteen single-query cap')
        commands=[]
        for t in targets:
    ...
    def move_actual(s,pose,label):
        ...
        _execute_control(scene,plan['controls'][0],label,arm='left')
        return plan['controls'][0]
    def extend(d,label):
        goal=np.array(_arm_eef_pose(scene,'left'));goal[2]+=d;move_actual(scene,goal,label)
    def tail():
        env=dict(old.__dict__);env.update(_plan_chain=plan_actual,_move_left=move_actual)
...
def run(manifest):
    ...
    for node in ast.walk(tree):
        if isinstance(node,ast.Compare) and isinstance(node.left,ast.Constant) and node.left.value==0 and len(node.comparators)==2:
            ...
            if isinstance(last,ast.Constant) and last.value==4:last.value=15;changes+=1
