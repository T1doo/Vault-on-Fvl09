"""One newly qualified recipe; three sequential plans from actual states."""
...
def execute(scene,proposal,out):
    ...
    if receipt is None:return {'pass':False,'earliest_failed_stage':'actual_postclose_lift_planner','events':events,'windows':windows,'plans':plans,'close_executed':True,'post_close_baseline_row':baseline}
...
def run(manifest):
    ...
    out=Path(manifest['jobs'][0]['output_namespace']);out.mkdir(parents=True,exist_ok=False);recipe=candidates()[0];proposal=json.loads((A/'F3_GEOMETRY_TOPDOWN_PROPOSAL_V1_20260906.json').read_text(encoding='utf-8'))['proposals'][0]
    h=helper();legacy=candidate_spec(recipe);adapter=h.adapter_for('F3',legacy,out/'adapter',SOURCE_SHA);ctx=_PinnedSapienRenderDeviceContextV1(adapter.scene(legacy,phase='NEW_TOPDOWN_MICRO',program=None));result=error=trace=None;before=after=None
    try:
        with ctx as handle:
            scene=handle.scene
            ...
            before=scene.planner_query_count
            try:
                binding=h.prepare_f3_scene(scene,adapter,recipe,_f3_scene_binding(recipe));write_new(out/'scene_binding.json',binding);result=execute(scene,proposal,out)
            finally:
                after=scene.planner_query_count;trace=h.save_trace(scene,out/'physical_trace.npz');write_new(out/'diagnostic_trace_receipt.json',trace)
    ...
    return {'schema_version':'cmf_f3_topdown_one_qualified_micro_terminal_v1','manifest_sha256':manifest['manifest_sha256'],'proposal_id':proposal['proposal_id'],'result':result,'error':error,'cleanup':cleanup,'trace':trace,
        ...
        'pass':known and error is None and cleanup is not None and cleanup.get('cleanup_safety_pass')is True,'micro_pass':result is not None and result['pass']}
