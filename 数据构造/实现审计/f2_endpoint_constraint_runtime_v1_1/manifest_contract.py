...
def load_manifest(path,*,execution=True,runner=False,post=False):
    ...
    if auth['decision']['F2']['existing_caps']['ik_problems']!=15:raise PermissionError('conformance scene scope')
    ...
    job=m['jobs'][0]
    for k,v in {'family':'F2','fresh_scene_cap':1,'trajectory_query_cap':4,'physical_attempt_cap':0,'ik_problem_cap':15,'timeout_seconds':3600}.items():
    ...
    prior=checked(m['prior_terminal_path']);g=checked(m['prior_guard_path'])
    if prior['fresh_scene_attempts']!=1 or prior['total_solver_problems']!=0 or g['task_owned_cleanup_pass'] is not True:raise ValueError('prior accounting/cleanup')
    if m['cumulative_scene_cap']!=2 or m['remaining_scene_count']!=1:raise ValueError('remaining cap')
    if runner:
        ...
        if start['manifest_sha256']!=m['manifest_sha256'] or start['guard_pid']!=os.getppid() or start['family']!='F2':raise PermissionError('Guard parent binding')
...
def load_and_validate_manifest_job(path,job_id,*,phase,require_execution_authorized,executable_role,executable_path):
    ...
    if phase==GUARD_ENTRY:
        ...
        if terminal['manifest_sha256']!=m['manifest_sha256'] or terminal['fresh_scene_attempts']!=1 or terminal['physical_execution_count']!=0:raise ValueError('terminal scope')
        if terminal['accounting_complete'] and (terminal['trajectory_queries']>4 or terminal['ik_problem_attempts']>15 or terminal['total_solver_problems']>19):raise ValueError('unapproved trajectory query')
