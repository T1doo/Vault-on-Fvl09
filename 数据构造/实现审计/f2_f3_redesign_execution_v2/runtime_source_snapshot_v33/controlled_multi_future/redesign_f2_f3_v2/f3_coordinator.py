"""Bounded coordinator for the first real F3 closure qualification attempt."""
...
def _process_snapshot() -> list[dict[str, Any]]:
    rows = []
    output = subprocess.run(["ps", "-eo", "pid=,ppid=,pgid=,lstart=,cmd="], text=True, stdout=subprocess.PIPE, check=True).stdout
    for line in output.splitlines():
        fields = line.strip().split(None, 7)
        if len(fields) == 8:
            rows.append({"pid": int(fields[0]), "ppid": int(fields[1]), "pgid": int(fields[2]), "start": " ".join(fields[3:7]), "cmd": fields[7]})
    return rows
...
def dispatch(*, vault_execution_dir: str | Path, output_root: str | Path, timeout_seconds: int = 300) -> dict[str, Any]:
    ...
    output = Path(output_root) / f"stage_B_f3_closure_{int(time.time())}"
    ...
    ledger = BudgetLedger(vault / "budget_ledger.jsonl", contract_sha256=contract_sha, parent_contract_sha256=contract.get("parent_contract_sha256"), ancestor_contract_sha256s=contract.get("ancestor_contract_sha256s"), task_id=contract["task_id"])
    wave_snapshot = live_snapshot()
    assignment = assign_ready_jobs([{"job_id": output.name, "root_id": "F3-A-closure-qualification"}], wave_snapshot)
    if not assignment["assignments"]:
        raise GPUGuardError("no fresh idle GPU for F3 closure qualification")
    ...
    atomic_write_json(output / "pre_guard_snapshot.json", {"wave_snapshot": wave_snapshot, "launch_snapshot": launch_snapshot, "assignment": selected, "card": card, "reservation": reservation})
    env = child_environment(selected["gpu_uuid"])
    env["CMF_GPU_GUARD_PHYSICAL_INDEX"] = str(selected["physical_gpu_index"])
    command = [sys.executable, "-m", "controlled_multi_future.redesign_f2_f3_v1.f3_closure_probe", "--output", str(output)]
    ...
    try:
        ...
        selected_post = next(item for item in post_snapshot["gpus"] if item["physical_index"] == selected["physical_gpu_index"])
        device_idle_observed = bool(selected_post["independently_fresh_idle"])
        post_reason = None
    except Exception as exc:
        post_snapshot = {"status": "UNAVAILABLE", "error": f"{type(exc).__name__}: {exc}"}
        device_idle_observed = False
        post_reason = str(exc)
    probe_path = output / "f3_closure_receipt.json"
    probe = _read(probe_path) if probe_path.exists() else None
    if probe is None:
        actual = {"solver_problems": reservation["solver_problems"], "fresh_scenes": reservation["fresh_scenes"], "action_scenes": reservation["action_scenes"], "collection_attempts": 0, "gpu_lease_seconds": math.ceil(elapsed)}
        unknown_consumption = True
    else:
        actual = {"solver_problems": int(probe.get("solver_problem_count", 0)), "fresh_scenes": 1, "action_scenes": 1, "collection_attempts": 0, "gpu_lease_seconds": math.ceil(elapsed)}
        raw_action_segments = int(probe.get("action_scene_count", 0))
    ...
    job_receipt = {
        ...
        "pre_snapshot_path": str(output / "pre_guard_snapshot.json"),
        "post_snapshot": post_snapshot,
        "actual": actual,
        "raw_action_segment_count": raw_action_segments if probe is not None else None,
        "unknown_consumption": unknown_consumption,
        "settlement_event_sha256": settlement["event_sha256"],
        "probe_receipt": probe,
        "process_snapshot_after": _process_snapshot(),
    }
    atomic_write_json(output / "job_receipt.json", job_receipt)
...
def main():
    ...
    result = dispatch(vault_execution_dir=args.vault_execution_dir, output_root=args.output_root, timeout_seconds=args.timeout_seconds)
