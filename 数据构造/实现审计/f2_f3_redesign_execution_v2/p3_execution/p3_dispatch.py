...
def main() -> int:
    ...
    allowed_states = {"READY_FIRST_TWO", "READY_FIRST_TWO_RECOVERY", "READY_F3_FIRST", "READY_F3_REMAINING", "READY_F3_B_FIRST", "READY_F3_B_REMAINING", "READY_F3_B_REMAINING_NEXT", "READY_F3_B_BASELINE"}
    ...
    elif wave_name.startswith("F3_B_REMAINING"):
        state["progress"]["f3_b_remaining"] = "PASSED" if wave_pass else "FAILED"
        if wave_pass:
            state["progress"]["completed_cells"] = int(state["progress"].get("completed_cells", 0)) + sum(int(item.get("actual_usage", {}).get("fresh_scenes", 0)) for item in results)
            state["status"] = "READY_F3_B_REMAINING_NEXT"
    elif wave_name == "F3_B_BASELINE_VHVH":
        state["progress"]["f3_b_baseline"] = "PASSED" if wave_pass else "FAILED"
        if wave_pass:
            state["progress"]["completed_cells"] = int(state["progress"].get("completed_cells", 0)) + sum(int(item.get("actual_usage", {}).get("fresh_scenes", 0)) for item in results)
            state["status"] = "READY_F3_B_BASELINE_COMPLETE"
    else:
