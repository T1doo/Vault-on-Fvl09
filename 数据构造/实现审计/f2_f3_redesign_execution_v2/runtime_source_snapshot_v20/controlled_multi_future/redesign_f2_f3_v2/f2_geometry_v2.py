...
def classify_relation(*, pose: Any, spec: dict[str, Any], contact: dict[str, bool] | None = None) -> dict[str, Any]:
    ...
    beside_region = bool(
        ...
        and np.all(np.abs(world_center[:2] - stand[:2]) >= np.asarray(f2["stand_half_xy"]) - world_half[:2])
    )
...
_SPEC_FIXTURE = {
    ...
    "scale_half_xy": [0.04, 0.06], "scale_top_z": 0.760, "beside_annulus_radius": [0.105, 0.205], "beside_table_z": 0.740,
