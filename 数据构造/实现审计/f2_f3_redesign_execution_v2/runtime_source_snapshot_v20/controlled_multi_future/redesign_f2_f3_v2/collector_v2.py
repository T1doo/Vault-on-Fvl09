...
def _closed_loop_event(scene: Any, realization: str, axis: str, label: str, receipt: dict[str, Any]) -> None:
    ...
    for phase, offset, hold in zip(("negative", "positive", "return"), (-amplitude, amplitude, 0.0), holds):
        target = center.copy(); target[main_axis] += offset; _move_arm(scene, target, f"{label}_{phase}", receipt); receipt["action_segment_count"] += 1; _wait_and_record_for_event(scene, hold)
    ...
    try:
        with managed_scene(RedesignF2Scene, args, receipt, f"f2-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if realization_id == "r_inv_path":
                ...
                detour_end = high.copy(); detour_end[1] -= 0.04
                receipt["realization_contract"]["path_variant_waypoints"] = {"start": detour_start.tolist(), "target_offset": detour_end.tolist(), "target": high.tolist()}
...
def run_root(*, output: str | Path, root_id: str, cell_keys: list[str] | None = None, existing_root: str | Path | None = None, collection: bool = True) -> dict[str, Any]:
    ...
    root_receipt["receipt_sha256"] = canonical_sha256(root_receipt); atomic_write_json(root_output / "root_receipt.json", root_receipt)
    if complete:
        ...
        root_receipt["receipt_sha256"] = canonical_sha256(root_receipt)
