...
def _variant_check(baseline: dict[str, Any], alternative: dict[str, Any]) -> dict[str, Any]:
    ...
    if kind == "path":
        ...
        holds_match = b_holds == expected_base and a_holds == expected_alt
        durations_match = len(b_segments) == 3 and len(a_segments) == 3 and all(int(a_segments[i].get("duration_rows", 0)) > int(b_segments[i].get("duration_rows", 0)) for i in range(3))
        changed = command_changed and len(a_suffix) != len(b_suffix) and variant_contract_pass and holds_match and durations_match
    else:
        changed = False
