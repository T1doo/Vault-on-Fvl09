...
class ExecutionLedgerV2:
                ...
                if idempotency_key:
                    for prior in events:
                        if prior.get("idempotency_key") == idempotency_key:
                            return prior
                if event_type not in {"BUDGET_OVERRUN", "ACCOUNTING_EXCEPTION"} and any(reserved[key] + consumed[key] > self.caps[key] for key in COUNTERS):
    ...
    def accounting_exception(self, job_id: str, reservation: Mapping[str, int], budget_charge: Mapping[str, int], *, metadata: Mapping[str, Any], idempotency_key: str) -> dict[str, Any]:
        """Close an unrecoverable reservation without claiming an exact usage."""
        reserved_amount = _counters(reservation, label="reservation")
        charge = _counters(budget_charge, label="budget_charge")
        totals = self.totals()
        reserved = {key: max(0, totals["reserved"][key] - reserved_amount[key]) for key in COUNTERS}
        consumed = {key: totals["consumed"][key] + charge[key] for key in COUNTERS}
        event_type = "BUDGET_OVERRUN" if any(consumed[key] > self.caps[key] for key in COUNTERS) else "ACCOUNTING_EXCEPTION"
        value = {**dict(metadata), "job_id": job_id, "reservation": reserved_amount, "budget_charge": charge, "exact_usage_claimed": False, "accounting_exception": True}
        return self.append(event_type=event_type, reserved_after=reserved, consumed_after=consumed, metadata=value, idempotency_key=idempotency_key)
