...
def build_beside_geometry_target_route(
    carry_pose: Any,
    target_pose: Any,
    *,
    route_offset_m: float = 0.12,
    ...
    """Build the evidence-driven route that omits the failed high waypoint."""
    carry = np.asarray(carry_pose, dtype=np.float64).reshape(-1)
    target = np.asarray(target_pose, dtype=np.float64).reshape(-1)
    if carry.shape != (7,) or target.shape != (7,):
        raise ValueError("beside geometry route poses must be seven-dimensional")
    if lateral_axis not in (0, 1):
        raise ValueError("beside geometry route lateral_axis must be 0 or 1")
    if not np.isfinite(carry).all() or not np.isfinite(target).all():
        raise ValueError("beside geometry route poses must be finite")
    if isinstance(route_offset_m, bool) or not np.isfinite(float(route_offset_m)) or float(route_offset_m) <= 0:
        raise ValueError("beside geometry route offset must be a positive finite number")
    side_at_current = carry.copy()
    side_at_current[lateral_axis] = float(carry[lateral_axis] + float(route_offset_m))
    if np.array_equal(side_at_current, carry):
        raise ValueError("beside geometry route collapsed its side waypoint")
    if not np.array_equal(side_at_current[np.arange(7) != lateral_axis], carry[np.arange(7) != lateral_axis]):
        raise ValueError("beside geometry route side waypoint changed non-lateral coordinates")
    return {
        "side_at_current": side_at_current.tolist(),
