...
def _state(scene: Any) -> dict[str, Any]:
    ...
    dynamic_role = "main_can" if "main_can" in roles else "bottle" if "bottle" in roles else None
    if dynamic_role is None or not all(role_velocity_available[dynamic_role].values()):
        raise ValueError("dynamic manipulation object lacks native linear/angular velocity readback")
...
def capture_t0(*, scene: Any, output: Path, root_id: str, cell_key: str, spec: dict[str, Any], rest_target: list[float] | None = None, trace_row: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    metadata = {
        ...
        "required_camera_names": list(MODEL_CAMERAS), "camera_names": sorted(images), "camera_images": image_meta, "camera_config": _camera_config(scene),
...
def read_bundle(path: Path) -> dict[str, Any]:
    ...
    checks = {
        ...
        "required_camera_names": metadata.get("required_camera_names") == list(MODEL_CAMERAS),
        "rgb_present": bool(images),
        "rgb_uint8_rectangular": all(v.ndim == 3 and v.shape[-1] == 3 and v.dtype == np.uint8 for v in images.values()),
        "rgb_camera_set": set(metadata.get("camera_names", [])) == set(MODEL_CAMERAS) and {key.removesuffix("__rgb") for key in images} == set(MODEL_CAMERAS),
        "rgb_camera_metadata_exact": set(metadata.get("camera_images", {})) == set(MODEL_CAMERAS),
        "rgb_hashes_match": all(metadata.get("camera_images", {}).get(key.removesuffix("__rgb"), {}).get("sha256") == _sha_bytes(value.tobytes()) for key, value in images.items()),
...
def model_envelopes(*, bundle: dict[str, Any], future: np.ndarray, candidates: list[dict[str, Any]], target: dict[str, Any]) -> dict[str, Any]:
    ...
    if bundle.get("checks", {}).get("pass") is not True:
