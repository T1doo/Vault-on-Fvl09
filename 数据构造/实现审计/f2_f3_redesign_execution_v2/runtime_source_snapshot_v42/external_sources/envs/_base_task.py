...
class Base_Task(gym.Env):
    ...
    def left_move_to_pose(
        ...
        # Preserve the planner's terminal mapping on failure.  The previous
        # bare return converted a real ``Fail + planner_diagnostics`` result
        # into ``None`` before the trace/audit layer could persist the query.
        # Returning the unchanged result still marks the task unsuccessful;
        # callers will refuse to execute a non-success control.
        if not isinstance(left_result, dict) or left_result.get("status") != "Success":
            self.plan_success = False
            return left_result
    ...
    def right_move_to_pose(
        ...
        if not isinstance(right_result, dict) or right_result.get("status") != "Success":
            self.plan_success = False
            return right_result
