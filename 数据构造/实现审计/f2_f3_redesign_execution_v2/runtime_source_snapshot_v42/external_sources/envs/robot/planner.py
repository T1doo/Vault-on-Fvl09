...
try:
    ...
    class CuroboPlanner:
        ...
        def _audit_value(value):
            """Convert a MotionGen scalar/enum to JSON-safe audit evidence."""
            ...
            if callable(item):
                try:
                    return CuroboPlanner._audit_value(item())
                except (RuntimeError, ValueError):
                    pass
            ...
            if callable(tolist):
                try:
                    ...
                    if isinstance(value, (list, tuple)):
                        return [CuroboPlanner._audit_value(entry) for entry in value]
                    return CuroboPlanner._audit_value(value)
                except (RuntimeError, ValueError):
                    pass
            return str(current)

        @classmethod
        def _audit_motion_result(cls, result, *, target_pose, start_joint_positions):
            fields = (
                "success", "status", "valid_query", "attempts", "used_graph",
                "position_error", "rotation_error", "solve_time", "total_time",
                "ik_time", "trajopt_time", "graph_time",
            )
            return {
                "schema_version": "cmf_motiongen_result_audit_v2",
                "fields": {field: cls._audit_value(getattr(result, field, None)) for field in fields},
                "target_pose_solver_frame": cls._audit_value(target_pose),
                "start_joint_positions_solver": cls._audit_value(start_joint_positions),
                "source": "conditional v2 planner audit before status flattening",
            }
        ...
        def plan_path(
            ...
            diagnostics = self._audit_motion_result(
                result,
            ...
            if result.success.item() == False:
                ...
                if diagnostics is not None:
                    res_result["planner_diagnostics"] = diagnostics
                return res_result
            else:
