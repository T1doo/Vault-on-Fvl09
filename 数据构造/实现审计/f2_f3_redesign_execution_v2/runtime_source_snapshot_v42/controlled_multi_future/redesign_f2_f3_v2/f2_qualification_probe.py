...
def run(*, output: str | Path, root_id: str, program_id: str, realization_id: str, f2_layout_id: str = "v4_beside_y_workspace", f2_route_mode: str = "three_segment_high", f2_lift_clearance_m: float | None = None) -> dict:
    output_path = Path(output)
    result = _f2_cell(output=output_path, root_id=root_id, program_id=program_id, realization_id=realization_id, artifact=None, collection=True, f2_layout_id=f2_layout_id, f2_route_mode=f2_route_mode, f2_lift_clearance_m=f2_lift_clearance_m)
    ...
    qualification = {
        ...
        "lift_clearance_m": result.get("lift_clearance_m"),
        "collection": True,
        "cell_receipt": result,
...
def main() -> int:
    ...
    parser.add_argument("--f2-lift-clearance-m", type=float)
    args = parser.parse_args()
    result = run(output=args.output, root_id=args.root_id, program_id=args.program_id, realization_id=args.realization_id, f2_layout_id=args.f2_layout_id, f2_route_mode=args.f2_route_mode, f2_lift_clearance_m=args.f2_lift_clearance_m)
