...
class ModelSpec:
    ...
    collision_bounds_local: tuple[tuple[float, float, float], tuple[float, float, float]]
    collision_support_point_local: tuple[float, float, float]
    ...
    def to_receipt(self) -> dict[str, Any]:
        return {
            ...
            "collision_bounds_local": [list(self.collision_bounds_local[0]), list(self.collision_bounds_local[1])],
            "collision_support_point_local": list(self.collision_support_point_local),
...
def _collision_bounds_local(path: Path, scale: Sequence[float]) -> tuple[tuple[float, float, float], tuple[float, float, float], tuple[float, float, float]]:
    """Read the scaled collision mesh bounds in the actor-local frame.

    Contact-point annotations describe where a gripper should touch an
    object.  They are not a support-plane measurement.  The F2 placement
    contract therefore derives its bottom point from the actual collision
    shapes.  This is kept in the metadata factory so the CPU scene contract
    and the runtime builder consume the same geometry.
    """
    try:
        import trimesh
        ...
        raise FactoryError(f"collision mesh bounds unavailable for {path}: {exc}") from exc
    if getattr(bounds, "shape", None) != (2, 3):
        raise FactoryError(f"collision mesh bounds have unexpected shape for {path}: {getattr(bounds, 'shape', None)}")
    lower = tuple(float(bounds[0][i]) * float(scale[i]) for i in range(3))
    upper = tuple(float(bounds[1][i]) * float(scale[i]) for i in range(3))
    if any(value != value or value in (float("inf"), float("-inf")) for value in (*lower, *upper)) or any(lower[i] > upper[i] for i in range(3)):
        raise FactoryError(f"collision mesh bounds are non-finite or inverted for {path}")
    # The frozen can/bottle assets use local Y as their long/up axis.  The
    # support point is the center of the lowest local-Y collision section;
    # the actor pose then rotates it into world Z at placement time.
    support = ((lower[0] + upper[0]) / 2.0, lower[1], (lower[2] + upper[2]) / 2.0)
...
def load_model_spec(asset_root: str | Path, model_name: str, *, model_id: int | None = 0, asset_id: str | None = None) -> ModelSpec:
    ...
    collision_lower, collision_upper, collision_support = _collision_bounds_local(collision, scale)
    return ModelSpec(
        ...
        collision_bounds_local=(collision_lower, collision_upper),
        collision_support_point_local=collision_support,
