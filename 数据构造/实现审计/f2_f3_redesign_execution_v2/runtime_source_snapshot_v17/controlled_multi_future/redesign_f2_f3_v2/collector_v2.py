...
def _move_arm(scene: Any, pose: Any, label: str, receipt: dict[str, Any]) -> None:
...
def _f2_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, artifact: Path | None, collection: bool) -> dict[str, Any]:
    ...
    spec = scene_spec(root_id); contract = planned_root_contract(root_id)
    receipt: dict[str, Any] = {"schema_version": "cmf_f2_cell_v2", "root_id": root_id, "family": "F2", "program_id": program_id, "realization_id": realization_id, "collection": collection, "formal_data": False, "stage0_data": False, "status": "RUNNING", "action_segment_count": 0, "solver_problem_count": 0, "scene_spec_sha256": spec["spec_sha256"], "candidate_set": candidate_set_for(spec)}
    initialize_cleanup_fields(receipt); started = time.monotonic(); scene = None
    receipt["realization_contract"] = {"r_pc": {"kind": "baseline", "path_variant": "direct_high_lateral", "motion_variant": "planner_default", "variant_applied": True}, "r_inv_path": {"kind": "path", "path_variant": "post_prefix_y_detour_40mm", "motion_variant": "planner_default", "variant_applied": False}, "r_inv_motion": {"kind": "motion", "path_variant": "direct_high_lateral", "motion_variant": "post_lateral_35_and_post_descent_40_frame_holds", "variant_applied": False}}[realization_id]
    args = _args("F2", output / "scene"); args.update({"seed": spec["scene_seed"], "task_name": f"cmf_f2_v2_{root_id}", "f2_layout_id": "v3", "scene_variant": spec["layout_variant"], "render_freq": 0, "save_data": False, "collect_data": False, "need_plan": True})
    try:
        with managed_scene(RedesignF2Scene, args, receipt, f"f2-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            receipt["t0_capture"] = capture_t0(scene=scene, output=output / "current", root_id=root_id, cell_key=f"{program_id}:{realization_id}", spec=spec, trace_row=scene.trace[0])
            ...
            if artifact is None:
                ...
                before = _eef(scene); lift = before.copy(); lift[2] += 0.16
            ...
            carried = _pose(scene.can); offset = carried[:3] - carry[:3]; f2 = spec["f2"]
            ...
            target = carry.copy(); target[:3] = desired - offset; receipt["target_binding"] = {"carried_object": carried.tolist(), "carried_eef": carry.tolist(), "grasp_offset": offset.tolist(), "desired_support_point": desired.tolist(), "support_identity": support_name, "target_eef": target.tolist()}; receipt["support_identity"] = support_name
            high = target.copy(); high[2] = carry[2]
            if realization_id == "r_inv_path":
                detour = high.copy(); detour[1] -= 0.04; _move_arm(scene, detour, f"v2_{program_id}_path_detour_start", receipt); receipt["action_segment_count"] += 1; _wait_and_record(scene, 20); detour_end = high.copy(); detour_end[1] -= 0.04; _move_arm(scene, detour_end, f"v2_{program_id}_path_detour_end", receipt); receipt["action_segment_count"] += 1; _wait_and_record(scene, 20); receipt["realization_contract"]["variant_applied"] = True
            ...
            exit_target = np.asarray(scene.robot.left_original_pose, dtype=float) if program_id == "beside" else (_eef(scene) + np.asarray([-0.15, 0.0, 0.10]))
        ...
        if scene is not None and getattr(scene, "trace", None) and "trace_path" not in receipt:
            try: receipt["trace"] = scene.save_trace(output / "trace.npz"); receipt["trace_path"] = str(output / "trace.npz")
...
def _f3_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, artifact: Path | None, prefix_meta: Path | None, collection: bool) -> dict[str, Any]:
    ...
    from .pilot_f3_root_probe import _closed_loop_event, _motion_metrics

    spec = scene_spec(root_id); receipt: dict[str, Any] = {"schema_version": "cmf_f3_cell_v2", "root_id": root_id, "family": "F3", "program_id": program_id, "realization_id": realization_id, "collection": collection, "formal_data": False, "stage0_data": False, "status": "RUNNING", "action_segment_count": 0, "solver_problem_count": 0, "scene_spec_sha256": spec["spec_sha256"], "candidate_set": candidate_set_for(spec), "realization_contract": {"kind": "baseline" if realization_id == "r_pc" else realization_id.removeprefix("r_inv_"), "path_variant": "direct_event_sequence" if realization_id != "r_inv_path" else "post_prefix_y_detour_30mm_round_trip", "motion_variant": "planner_default" if realization_id != "r_inv_motion" else "absolute_symmetric_planner_events_with_extended_holds", "variant_applied": realization_id == "r_pc"}}
    initialize_cleanup_fields(receipt); started = time.monotonic(); scene = None
    ...
    try:
        with managed_scene(RedesignF3Scene, args, receipt, f"f3-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if artifact is None:
                ...
                if evidence:
                    start = int(evidence["start_row"]); metrics = _motion_metrics(scene.trace[start: prefix_end + 1], "V"); receipt["prefix_event_evidence"] = {"start_row": start, "end_row": prefix_end, "metrics": metrics, "verifier": verify_realized_motion_metrics({"shared_V": metrics}, {"motion_min_axis_amplitude_m": 0.04, "motion_max_off_axis_m": 0.015, "motion_max_return_error_m": 0.015, "motion_max_orientation_drift": 0.05, "motion_min_contact_fraction": 0.95, "motion_max_contact_break_count": 0})}
            ...
            if realization_id == "r_inv_path":
                detour = _eef(scene); detour[1] -= 0.03; _move_arm(scene, detour, "v2_path_detour", receipt); receipt["action_segment_count"] += 1; _wait_and_record(scene, 20); back = detour.copy(); back[1] += 0.03; _move_arm(scene, back, "v2_path_return", receipt); receipt["action_segment_count"] += 1; _wait_and_record(scene, 20); receipt["realization_contract"]["variant_applied"] = True
            suffix = {"VVHH": ("V", "H", "H"), "VHVH": ("H", "V", "H"), "VHHV": ("H", "H", "V")}[program_id]; receipt["expected_suffix_event_order"] = list(suffix); receipt["event_segments"] = []
            for index, axis in enumerate(suffix):
                start = len(scene.trace) - 1; _closed_loop_event(scene, realization_id, axis, f"v2_suffix_{index}_{axis}", receipt); end = len(scene.trace) - 1; receipt["event_segments"].append({"event_index": index + 1, "axis": axis, "start_row": start, "end_row": end, "metrics": _motion_metrics(scene.trace[start: end + 1], axis)})
            receipt["event_verifier"] = verify_realized_motion_metrics({f"suffix_{e['event_index']}_{e['axis']}": e["metrics"] for e in receipt["event_segments"]}, {"motion_min_axis_amplitude_m": 0.04, "motion_max_off_axis_m": 0.015, "motion_max_return_error_m": 0.015, "motion_max_orientation_drift": 0.05, "motion_min_contact_fraction": 0.95, "motion_max_contact_break_count": 0})
...
def _save_prefix(root_output: Path, receipt: dict[str, Any]) -> Path | None:
    ...
    trace = np.load(trace_path, allow_pickle=False); n = int(end) + 1; artifact = root_output / "prefix_artifact.npz"
...
def run_root(*, output: str | Path, root_id: str, cell_keys: list[str] | None = None, existing_root: str | Path | None = None, collection: bool = True) -> dict[str, Any]:
    root_output = Path(output); root_output.mkdir(parents=True, exist_ok=False); contract = planned_root_contract(root_id); spec = scene_spec(root_id); expected = expected_cells(root_id); expected_keys = [f"{c['program_id']}:{c['realization_id']}" for c in expected]
    ...
    if existing_root is not None:
        ...
        source_sha = sha256_file(source_root / "root_receipt.json"); reusable = reusable_cells(source, root_id, cell_keys={(c["program_id"], c["realization_id"]) for c in expected if f"{c['program_id']}:{c['realization_id']}" not in set(requested)})
    ...
    for key in order:
        ...
        if contract["family"] == "F2": receipt = _f2_cell(output=cell_dir, root_id=root_id, program_id=program, realization_id=realization, artifact=artifact, collection=collection)
        ...
        if artifact is None and receipt.get("prefix_sha256"):
            artifact = _save_prefix(root_output, receipt); prefix_meta = root_output / "prefix_evidence.json" if (root_output / "prefix_evidence.json").is_file() else None
    cells = []
    ...
    complete = len(cells) == 6 and all(c.get("status") == "cell_pass" for c in cells)
    root_receipt = {"schema_version": "cmf_f2_f3_root_v2", "root_id": root_id, "family": contract["family"], "contract": contract, "scene_spec": spec, "scene_spec_sha256": spec["spec_sha256"], "cells": cells, "execution_order": order, "completed_cell_keys": [f"{c.get('program_id')}:{c.get('realization_id')}" for c in cells if c.get("status") == "cell_pass"], "root_complete": complete, "accepted": complete, "status": "ACCEPTED" if complete else "INCOMPLETE", "new_cell_count": len(generated), "collection_count": sum(c.get("collection") is True for c in generated.values()), "reused_from_root_receipt_sha256": source_sha, "candidate_set": candidate_set_for(spec), "current_anchor_cross_branch": "PENDING_UNTIL_SIX_CELLS" if not complete else "REQUIRES_INDEPENDENT_FINALIZER"}
    root_receipt["receipt_sha256"] = canonical_sha256(root_receipt); atomic_write_json(root_output / "root_receipt.json", root_receipt); return root_receipt


def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--output", required=True); parser.add_argument("--root-id", required=True, choices=("F2-A-v2", "F2-B-v2", "F3-A-v2", "F3-B-v2")); parser.add_argument("--cell-keys"); parser.add_argument("--existing-root"); parser.add_argument("--no-collection", action="store_true"); args = parser.parse_args(); keys = args.cell_keys.split(",") if args.cell_keys else None; result = run_root(output=args.output, root_id=args.root_id, cell_keys=keys, existing_root=args.existing_root, collection=not args.no_collection); print(json.dumps({"root_id": result["root_id"], "status": result["status"], "completed_cell_keys": result["completed_cell_keys"], "receipt_sha256": result["receipt_sha256"]}, ensure_ascii=False)); failures = any(cell.get("status") not in {"cell_pass", "PENDING"} for cell in result["cells"]); raise SystemExit(0 if result["accepted"] or (result["status"] == "INCOMPLETE" and not failures) else 1)
