...
from .f2_geometry_v2 import _world_geometry, build_beside_route_waypoints, verify_expected_relation
...
def _f2_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, artifact: Path | None, collection: bool, f2_layout_id: str | None = None) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {"schema_version": "cmf_f2_cell_v2", "root_id": root_id, "family": "F2", "program_id": program_id, "realization_id": realization_id, "collection": collection, "formal_data": False, "stage0_data": False, "status": "RUNNING", "action_segment_count": 0, "solver_problem_count": 0, "layout_id": spec["f2"]["layout_id"], "scene_spec_sha256": spec["spec_sha256"], "candidate_set": candidate_set_for(spec)}
    ...
    try:
        with managed_scene(RedesignF2Scene, args, receipt, f"f2-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if program_id == "beside":
                ...
                for label, waypoint in (("beside_side_at_current", route_far), ("beside_side_at_target", route_target), ("beside_target_y", route_final)):
...
def run_root(*, output: str | Path, root_id: str, cell_keys: list[str] | None = None, existing_root: str | Path | None = None, collection: bool = True, f2_layout_id: str | None = None) -> dict[str, Any]:
    ...
    for key in order:
        ...
        if contract["family"] == "F2": receipt = _f2_cell(output=cell_dir, root_id=root_id, program_id=program, realization_id=realization, artifact=artifact, collection=collection, f2_layout_id=spec["f2"]["layout_id"])
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--output", required=True); parser.add_argument("--root-id", required=True, choices=("F2-A-v2", "F2-B-v2", "F3-A-v2", "F3-B-v2")); parser.add_argument("--cell-keys"); parser.add_argument("--existing-root"); parser.add_argument("--no-collection", action="store_true"); parser.add_argument("--f2-layout-id", choices=("v4_beside_y_workspace", "v5_beside_y_lower")); args = parser.parse_args(); keys = args.cell_keys.split(",") if args.cell_keys else None; result = run_root(output=args.output, root_id=args.root_id, cell_keys=keys, existing_root=args.existing_root, collection=not args.no_collection, f2_layout_id=args.f2_layout_id); print(json.dumps({"root_id": result["root_id"], "status": result["status"], "completed_cell_keys": result["completed_cell_keys"], "receipt_sha256": result["receipt_sha256"]}, ensure_ascii=False)); failures = any(cell.get("status") not in {"cell_pass", "PENDING"} for cell in result["cells"]); partial_ok = result["status"] == "INCOMPLETE" and not failures and not result.get("stop_reason"); raise SystemExit(0 if result["accepted"] or partial_ok else 1)
