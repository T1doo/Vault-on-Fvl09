...
def run(*, output: str | Path, cell_receipt: str | Path, trace: str | Path, stand_xs: list[float] | None = None) -> dict[str, Any]:
    ...
    target_binding = source_receipt.get("target_binding") or {}
    target_value = np.asarray(target_binding.get("target_eef", saved_eef), dtype=np.float64).reshape(-1)
    if target_value.shape != (7,):
        target_value = saved_eef.copy()
    descent = target_value.copy()
    lateral = descent.copy(); lateral[2] = float(saved_eef[2])
    corrected_route = build_beside_route_waypoints(saved_eef, target_value, route_offset_m=0.12, lateral_axis=1)
    variants: list[tuple[str, np.ndarray, np.ndarray]] = [
        ("original", lateral, descent),
        ("corrected_side_then_transport", np.asarray(corrected_route["side_at_current"], dtype=np.float64), np.asarray(corrected_route["side_at_target"], dtype=np.float64)),
        ("corrected_transport_then_target", np.asarray(corrected_route["side_at_target"], dtype=np.float64), np.asarray(corrected_route["target"], dtype=np.float64)),
    ]
    support_offset = np.asarray(target_binding.get("support_offset", [0.0, 0.0, 0.0]), dtype=np.float64).reshape(-1)
    if support_offset.shape != (3,) or not np.isfinite(support_offset).all():
        support_offset = np.zeros(3, dtype=np.float64)
    for stand_x in stand_xs or []:
        desired = np.asarray([float(stand_x), -0.18, 0.740], dtype=np.float64)
        candidate_descent = target_value.copy(); candidate_descent[:3] = desired - support_offset; candidate_lateral = candidate_descent.copy(); candidate_lateral[2] = saved_eef[2]
