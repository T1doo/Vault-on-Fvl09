...
class Robot:
    ...
    def left_plan_path(
        ...
        if self.communication_flag:
            ...
            if bool(getattr(self, "_cmf_planner_audit", False)):
                message["audit"] = True
            self.left_conn.send(message)
            return self.left_conn.recv()
        else:
            kwargs = {"constraint_pose": constraint_pose, "arms_tag": "left"}
            if bool(getattr(self, "_cmf_planner_audit", False)) and hasattr(self.left_planner, "motion_gen"):
                kwargs["audit"] = True
            return self.left_planner.plan_path(now_qpos, trans_target_pose, **kwargs)
    ...
    def right_plan_path(
        ...
        if self.communication_flag:
            ...
            if bool(getattr(self, "_cmf_planner_audit", False)):
                message["audit"] = True
            self.right_conn.send(message)
            return self.right_conn.recv()
        else:
            kwargs = {"constraint_pose": constraint_pose, "arms_tag": "right"}
            if bool(getattr(self, "_cmf_planner_audit", False)) and hasattr(self.right_planner, "motion_gen"):
                kwargs["audit"] = True
            return self.right_planner.plan_path(now_qpos, trans_target_pose, **kwargs)
        ...
        try:
            ...
            if msg["cmd"] == "plan_path":
                result = planner.plan_path(
                    ...
                    audit=bool(msg.get("audit", False)),
                )
                conn.send(result)
