...
class V2ContractTests(unittest.TestCase):
    def test_ab_specs_are_real_layout_variants(self):
        ...
        self.assertAlmostEqual(a["f2"]["beside_target_xy"][0], 0.06)
    ...
    def test_beside_is_not_stand_top(self):
        ...
        quaternion = np.asarray([0.70710678, 0.70710678, 0.0, 0.0])
        support_offset = _rotation(quaternion) @ np.asarray(spec["f2"]["support_point_local_xyz"])
        support_target = np.asarray([*spec["f2"]["beside_target_xy"], spec["f2"]["beside_table_z"]])
        legal = verify_expected_relation(program_id="beside", pose=np.r_[support_target - support_offset, quaternion], spec=spec)
        self.assertTrue(legal["pass"])
        self.assertTrue(legal["separated_from_stand"])

    def test_beside_route_has_distinct_side_and_transport_waypoints(self):
        from controlled_multi_future.redesign_f2_f3_v2.f2_geometry_v2 import build_beside_route_waypoints

        carry = np.asarray([-0.08, -0.18, 1.09, 0.7, 0.0, 0.7, 0.0])
        target = np.asarray([0.24, -0.186, 0.88, 0.7, 0.0, 0.7, 0.0])
        route = build_beside_route_waypoints(carry, target)
        side_at_current = np.asarray(route["side_at_current"])
        side_at_target = np.asarray(route["side_at_target"])
        final = np.asarray(route["target"])
        self.assertFalse(np.array_equal(side_at_current, side_at_target))
        self.assertTrue(np.array_equal(side_at_current[[0, 2, 3, 4, 5, 6]], carry[[0, 2, 3, 4, 5, 6]]))
        self.assertAlmostEqual(side_at_target[1], side_at_current[1])
        self.assertAlmostEqual(side_at_target[0], target[0])
        self.assertAlmostEqual(final[1], target[1])
        self.assertAlmostEqual(final[2], carry[2])

    def test_beside_direct_geometry_route_keeps_side_clearance(self):
        from controlled_multi_future.redesign_f2_f3_v2.f2_geometry_v2 import build_beside_geometry_target_route

        carry = np.asarray([-0.08, -0.18, 1.09, 0.7, 0.0, 0.7, 0.0])
        target = np.asarray([0.06, -0.188, 0.934, 0.7, 0.0, 0.7, 0.0])
        route = build_beside_geometry_target_route(carry, target)
        side = np.asarray(route["side_at_current"])
        self.assertTrue(np.allclose(side[[0, 2]], carry[[0, 2]]))
        self.assertAlmostEqual(side[1], carry[1] + 0.12)
        self.assertTrue(np.allclose(route["target"], target))

    def test_f3_retiming_candidate_scales_motion_instead_of_hold(self):
        from controlled_multi_future.redesign_f2_f3_v2.f3_retiming import retiming_unit_checks

        self.assertTrue(all(retiming_unit_checks().values()))
    ...
    def test_accounting_exception_closes_unknown_reservation_without_claiming_exact_usage(self):
        from controlled_multi_future.redesign_f2_f3_v2.execution_ledger_v2 import ExecutionLedgerV2
        with tempfile.TemporaryDirectory() as directory:
            caps = {"fresh_scenes": 10, "action_scenes": 10, "collection_attempts": 10, "solver_problems": 100, "gpu_lease_seconds": 1000}
            ledger = ExecutionLedgerV2(Path(directory) / "ledger.jsonl", contract_sha256="contract-v2", task_id="task-v2", caps=caps)
            reservation = {"fresh_scenes": 5, "action_scenes": 5, "collection_attempts": 5, "solver_problems": 60, "gpu_lease_seconds": 900}
            ledger.reserve("job-unknown", reservation, idempotency_key="reserve-unknown")
            event = ledger.accounting_exception("job-unknown", reservation, {"fresh_scenes": 1, "action_scenes": 5, "collection_attempts": 1, "solver_problems": 60, "gpu_lease_seconds": 900}, metadata={"actual_gpu_lease_seconds": None, "exact_measurement_available": False}, idempotency_key="exception-unknown")
            self.assertEqual(event["event_type"], "ACCOUNTING_EXCEPTION")
            self.assertFalse(event["metadata"]["exact_usage_claimed"])
            totals = ledger.totals(); self.assertEqual(totals["reserved"], {key: 0 for key in caps}); self.assertEqual(totals["consumed"]["gpu_lease_seconds"], 900)

    def test_dispatcher_normalizes_cell_keys_before_launch(self):
        source = Path("/nfs_share/lijunhui/Vault-on-Fvl09/数据构造/实现审计/f2_f3_redesign_execution_v2/p3_execution/p3_dispatch.py").read_text(encoding="utf-8")
        self.assertIn("def _normalise_job", source)
        self.assertLess(source.index("def _normalise_job"), source.index("subprocess.Popen"))
        self.assertLess(source.index("end_evidence.json"), source.index("ledger.settle"))

    def test_real_collector_cli_binds_f2_route_before_scene_boundary(self):
        from controlled_multi_future.redesign_f2_f3_v2 import collector_v2

        seed = Path("/nfs_share/lijunhui/Robotwin2/datasets/f2_f3_observation_contract_repair_p4/p4-f2-a-v6-seed-root")
        self.assertTrue((seed / "root_receipt.json").is_file())
        keys = "beside:r_pc,beside:r_inv_path,inside:r_pc,inside:r_inv_path,on:r_pc,on:r_inv_path"
        captured = {}

        def scene_boundary(**kwargs):
            captured.update(kwargs)
            raise RuntimeError("SCENE_CREATION_BOUNDARY")
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            argv = [
                ...
                "--root-id", "F2-A-v2",
                "--cell-keys", keys,
                "--existing-root", str(seed),
                "--f2-layout-id", "v5_beside_y_lower",
                "--f2-route-mode", "side_then_geometry_target",
            ]
            with mock.patch.object(sys, "argv", argv), mock.patch.object(collector_v2, "_f2_cell", side_effect=scene_boundary):
                with self.assertRaisesRegex(RuntimeError, "SCENE_CREATION_BOUNDARY"):
                    collector_v2.main()
            self.assertEqual(captured["root_id"], "F2-A-v2")
            self.assertEqual(captured["f2_layout_id"], "v5_beside_y_lower")
            self.assertEqual(captured["f2_route_mode"], "side_then_geometry_target")
            self.assertTrue((output / "prefix_artifact.npz").is_file())
