...
def _f2_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, artifact: Path | None, collection: bool, f2_layout_id: str | None = None, f2_route_mode: str | None = None) -> dict[str, Any]:
    ...
    receipt: dict[str, Any] = {"schema_version": "cmf_f2_cell_v2", "root_id": root_id, "family": "F2", "program_id": program_id, "realization_id": realization_id, "collection": collection, "formal_data": False, "stage0_data": False, "status": "RUNNING", "action_segment_count": 0, "solver_problem_count": 0, "layout_id": spec["f2"]["layout_id"], "route_mode": route_mode, "scene_spec_sha256": spec["spec_sha256"], "candidate_set": candidate_set_for(spec)}
        ...
        with managed_scene(RedesignF2Scene, args, receipt, f"f2-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if program_id == "beside":
                ...
                if route_mode == "side_then_geometry_target":
                    route = build_beside_geometry_target_route(carry, target, route_offset_m=0.12, lateral_axis=1)
