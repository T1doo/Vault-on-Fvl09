...
def build_budget_request(output: Path, *, authorize: bool = False, source_snapshot_manifest: Path | None = None, cpu_gate: dict | None = None) -> dict:
    roots = {}
    for root_id in ("F2-A-v2", "F2-B-v2", "F3-A-v2", "F3-B-v2"):
    ...
    snapshot = source_snapshot_manifest or Path("/nfs_share/lijunhui/Vault-on-Fvl09/数据构造/实现审计/f2_f3_redesign_execution_v2/runtime_source_snapshot_v23/SNAPSHOT_MANIFEST.json")
    value = {
        "schema_version": "cmf_f2_f3_v2_complete_budget_request_v1",
        "status": "AUTHORIZED_FOR_P3_AFTER_CPU_GATES" if authorize else "REQUEST_ONLY_NOT_AUTHORIZED",
    ...
    if snapshot.is_file():
        value["source_snapshot_manifest"] = str(snapshot)
        value["source_snapshot_manifest_sha256"] = sha256_file(snapshot)
    if authorize:
        if not cpu_gate or cpu_gate.get("pass") is not True:
            raise ValueError("physical authorization requires a passing CPU gate receipt")
        value["physical_execution_authorized"] = True
        value["authorization"] = {"basis": "user-approved latest review scope", "cpu_gate": cpu_gate}
...
def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--budget-request", required=True); parser.add_argument("--authorize", action="store_true"); parser.add_argument("--source-snapshot"); parser.add_argument("--cpu-gate"); args = parser.parse_args(); cpu_gate = json.loads(Path(args.cpu_gate).read_text(encoding="utf-8")) if args.cpu_gate else None; value = build_budget_request(Path(args.budget_request), authorize=args.authorize, source_snapshot_manifest=Path(args.source_snapshot) if args.source_snapshot else None, cpu_gate=cpu_gate); print(json.dumps({"path": args.budget_request, "request_sha256": value["request_sha256"], "physical_execution_authorized": value["physical_execution_authorized"]}, ensure_ascii=False))
