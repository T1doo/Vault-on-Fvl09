...
from .f2_geometry_v2 import _world_geometry, verify_expected_relation
    ...
    try:
        with managed_scene(RedesignF2Scene, args, receipt, f"f2-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if program_id == "beside":
                # The direct high lateral target can be rejected by the full
                # arm/environment planner even though the object geometry is
                # legal.  Keep the same target and grasp, but route at the
                # declared high clearance around the stand before returning to
                # the target y coordinate.
                route_y = float(carry[1] + 0.12)
                route_far = high.copy(); route_far[1] = route_y
                route_target = route_far.copy(); route_target[0] = high[0]
                receipt["beside_route_waypoints"] = {"far_y": route_far.tolist(), "target_far_y": route_target.tolist(), "target": high.tolist(), "clearance_height_m": float(high[2])}
                for label, waypoint in (("beside_far_y", route_far), ("beside_target_far_y", route_target), ("beside_target_y", high)):
            ...
            if realization_id == "r_inv_path":
                receipt.setdefault("variant_waypoints_actual", []).append({"label": "target", "target": high.tolist(), "trace_row": len(scene.trace) - 1})
    ...
    try:
        with managed_scene(RedesignF3Scene, args, receipt, f"f3-v2-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            for index, axis in enumerate(suffix):
                start = len(scene.trace) - 1; _closed_loop_event(scene, realization_id, axis, f"v2_suffix_{index}_{axis}", receipt); end = len(scene.trace) - 1; hold_record = receipt.get("event_hold_records", [])[-1]; receipt["event_segments"].append({"event_index": index + 1, "axis": axis, "start_row": start, "end_row": end, "duration_rows": end - start + 1, "duration_seconds": float((end - start) * scene.simulator_timestep_seconds), "hold_frames": hold_record.get("hold_frames"), "metrics": _motion_metrics(scene.trace[start: end + 1], axis)})
