...
def run(*, output: str | Path, root_id: str, program_id: str, realization_id: str, f2_layout_id: str = "v4_beside_y_workspace") -> dict:
    output_path = Path(output)
    result = _f2_cell(output=output_path, root_id=root_id, program_id=program_id, realization_id=realization_id, artifact=None, collection=True, f2_layout_id=f2_layout_id)
...
def main() -> int:
    ...
    result = run(output=args.output, root_id=args.root_id, program_id=args.program_id, realization_id=args.realization_id, f2_layout_id=args.f2_layout_id)
