...
_F2_LAYOUT_IDS = {"v4_beside_y_workspace", "v5_beside_y_lower"}


def scene_spec(root_id: str, *, f2_layout_id: str | None = None) -> dict[str, Any]:
    ...
    if family == "F2":
        ...
        selected_layout = "v4_beside_y_workspace" if f2_layout_id is None else str(f2_layout_id)
        if selected_layout not in _F2_LAYOUT_IDS:
            raise ValueError(f"unknown F2 layout candidate: {selected_layout}")
        ...
        f2["layout_id"] = selected_layout
        f2["stand_center_xyz"] = [0.06, 0.08, 0.79]
        # v5 is the one finite follow-up candidate authorized after v4 failed
        # at the second side waypoint.  It keeps the reference stand off the
        # scale, puts the legal target on the lower-y table workspace, and
        # derives the box bounds from the new box center.
        if selected_layout == "v5_beside_y_lower":
            f2["box_center_xyz"] = [-0.24, -0.18, 0.755]
            f2["stand_center_xyz"] = [0.14, -0.06, 0.79]
            f2["box_inner_lower_xyz"] = [-0.35, -0.29, 0.760]
            f2["box_inner_upper_xyz"] = [-0.13, -0.07, 0.855]
        asset = load_model_spec(ASSET_ROOT, "071_can", model_id=0, asset_id="f2_can_v2")
        f2["asset_binding"] = asset.to_receipt()
        f2["collision_bounds_local_xyz"] = [list(asset.collision_bounds_local[0]), list(asset.collision_bounds_local[1])]
        f2["support_point_local_xyz"] = list(asset.collision_support_point_local)
        f2["support_point_source"] = "collision mesh local-Y lower section after effective_scale; transformed by actual actor quaternion"
        f2["grasp_point_local_xyz"] = list(asset.semantic_points.get("contact_0", (0.0, 0.0, 0.0)))
        f2["grasp_point_source"] = "asset model_data contact_points_pose[0] after effective_scale; grasp-only semantic point"
        ...
        f2["beside_target_xy"] = [0.06 + shift, -0.18] if f2["layout_id"] == "v5_beside_y_lower" else [0.06, -0.04]
