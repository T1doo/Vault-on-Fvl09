...
_V2_REALIZATION_REQUIREMENTS = {
    "r_pc": {
        "kind": "baseline",
        "variant_name": "direct_frozen_control",
    },
    "r_inv_path": {
        "kind": "path",
        "variant_name": "post_prefix_lateral_detour",
        "waypoint_count": 3,
        "detour_axis": "y",
        "detour_offset_m": -0.040,
        "required_waypoint_labels": ("detour_start", "detour_target_offset", "target"),
    },
    "r_inv_motion": {
        "kind": "motion",
        "variant_name": "extended_event_holds",
        "baseline_hold_frames": (20, 20, 20),
        "alternative_hold_frames": (30, 35, 30),
        "required_hold_delta_frames": (10, 15, 10),
    },
}
...
def planned_root_contract(root_id: str) -> dict[str, Any]:
    ...
    return {
        ...
        "realization_requirements": {
            realization: dict(_V2_REALIZATION_REQUIREMENTS[realization])
            for realization in ("r_pc", realization)
        },
    }


def frozen_realization_requirements(root_id: str, realization_id: str) -> dict[str, Any]:
    """Return the immutable variant contract used by collector and finalizer."""
    contract = planned_root_contract(root_id)
    if realization_id not in contract["realization_ids"]:
        raise PilotContractError("realization is outside the frozen root contract")
    result = dict(contract["realization_requirements"][realization_id])
    if root_id.startswith("F3") and realization_id == "r_inv_path":
        result["detour_offset_m"] = -0.030
    return result
