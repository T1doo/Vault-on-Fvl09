...
class ExecutionLedgerV2:
    ...
    def __init__(self, path: str | Path, *, contract_sha256: str, task_id: str, caps: Mapping[str, int]):
        self.path = Path(path)
    ...
    def _read_unlocked(self) -> list[dict[str, Any]]:
        ...
        with self.path.open(encoding="utf-8") as stream:
            for line_number, line in enumerate(stream, 1):
                ...
                if event.get("task_id") != self.task_id or event.get("contract_sha256") != self.contract_sha256:
