...
def dispatch(*, vault_execution_dir: str | Path, output_root: str | Path, root_id: str = "F3-A", timeout_seconds: int = 1800, existing_root: str | Path | None = None, programs: list[str] | tuple[str, ...] | None = None, realizations: list[str] | tuple[str, ...] | None = None, cell_keys: list[str] | tuple[str, ...] | None = None, qualification: bool = False, build_prefix: bool = False, single_cell: tuple[str, str] | None = None, layout_id: str = "v1") -> dict[str, Any]:
    ...
    if single_cell is not None and (existing_root is not None or programs is not None or realizations is not None or cell_keys is not None):
    ...
    if root_id.startswith("F2") and single_cell is None:
        ...
        if existing_root is not None:
            ...
            if cell_keys is not None:
                command.extend(["--cell-keys", ",".join(cell_keys)])
        if qualification:
    ...
    partial_receipts = [_read(path) for path in partial_cells]
    if probe:
        ...
        actual = {"solver_problems": sum(int(item.get("solver_problem_count", 0)) for item in partial_receipts), "fresh_scenes": len(partial_cells), "action_scenes": len(partial_cells), "collection_attempts": 0 if qualification else len(partial_cells), "gpu_lease_seconds": math.ceil(elapsed)}
    unknown = root is None and probe is None and not partial_receipts; settlement = ledger.settle(output.name, reservation, actual, idempotency_key=f"settle:{output.name}")
...
def main():
    ...
    result = dispatch(vault_execution_dir=args.vault_execution_dir, output_root=args.output_root, root_id=args.root_id, timeout_seconds=args.timeout_seconds, existing_root=args.existing_root, programs=programs, realizations=realizations, cell_keys=cell_keys, qualification=args.qualification, build_prefix=args.build_prefix, single_cell=single_cell, layout_id=args.layout_id)
