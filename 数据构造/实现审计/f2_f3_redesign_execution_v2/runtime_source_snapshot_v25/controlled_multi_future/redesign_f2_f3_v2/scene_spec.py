...
def scene_spec(root_id: str) -> dict[str, Any]:
    ...
    if family == "F2":
        ...
        # Keep the point inside the frozen beside annulus while leaving a
        # wider lateral clearance for the real end-effector sweep around the
        # stand.  This is the single bounded recovery change for the first
        # beside validation attempt.
        f2["beside_target_xy"] = [float(f2["stand_center_xyz"][0] + 0.160), float(f2["stand_center_xyz"][1])]
