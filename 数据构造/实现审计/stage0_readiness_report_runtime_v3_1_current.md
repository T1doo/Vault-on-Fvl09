...
Runtime-v3_1 已完成 GPT 指定的 GPU 前 CPU/code 加固，并实现 concrete lazy-import RoboTwin adapter/runner。70/70 tests 与 root-cpu4 synthetic evidence 通过。
...
7. runtime-v3_1 budget 仍 `proposed_for_user_review / approved=false / frozen=false`；
8. 不存在合法 GPU authorization receipt。
