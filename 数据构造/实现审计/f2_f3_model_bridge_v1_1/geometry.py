...
def capture_actor_geometry(actor,role,planner,arm='left'):
    ...
    for body in bodies:
        ...
        for shape in body.get_collision_shapes():
            ...
            value={'name':role+'__'+str(len(shapes)+len(body_shapes)),'role':role,'actor_name':entity.get_name(),'kind':kind,
...
def make_world(export):
    ...
    if len(names)!=len(set(names)):raise ValueError('duplicate collision shape names before solver load')
...
def exact_shape_pairs(robot_shapes,world_shapes):
    ...
    for left,a in robots:
        for right,b in world:
            ...
            witness=0.0
            if hit:
                lm=trimesh.Trimesh(vertices=left['vertices'],faces=left['faces'],process=True);lm.apply_transform(matrix(left['solver_pose']))
                rm=trimesh.Trimesh(vertices=right['vertices'],faces=right['faces'],process=True);rm.apply_transform(matrix(right['solver_pose']))
                if rm.is_watertight:witness=max(witness,float(np.max(trimesh.proximity.signed_distance(rm,lm.vertices))))
                if lm.is_watertight:witness=max(witness,float(np.max(trimesh.proximity.signed_distance(lm,rm.vertices))))
            rows.append({'robot_shape':left['name'],'obstacle_shape':right['name'],'role':right['role'],'physical_collision_filter_enabled':enabled,
                ...
                'vertex_penetration_witness_m':witness,'definite_open_grasp_geometry_violation':hit and (right['role'] in ('table','pad') or witness>1e-5),
