...
from model_apply import bind_actual_solver_base
...
def replay_retained_geometry(saved,scene,state,planner):
    from geometry import matrix,pose7
    from transforms import world_obstacle_to_solver_frame,t3d
    shapes=[];counts={};base=matrix(world_obstacle_to_solver_frame(planner,[0,0,0,1,0,0,0]))
    for original in saved['shapes']:
        role=original['role']
        if role not in ('table','pad','bottle'):continue
        s=copy.deepcopy(original);s.pop('geometry_sha256',None)
        idx=counts.get(role,0);counts[role]=idx+1;s['name']=role+'__'+str(idx)
        if role=='bottle':s['actor_world_pose']=state['bottle_pose']
        transform=base@matrix(s['actor_world_pose'])@matrix(s['shape_local_pose'])
        s['solver_pose']=np.r_[transform[:3,3],t3d.quaternions.mat2quat(transform[:3,:3])].tolist()
        s['geometry_origin']='retained actual asset13 collision capture, rigid pose replay; no new asset13 scene'
        s['body_aabb_validation_scope']='original measured local geometry; rigid reprojection does not rescale vertices'
        s['geometry_sha256']=digest(s);shapes.append(s)
    for link in scene.robot.right_entity.get_links():
        if link.get_name().startswith('fr_link'):shapes+=capture_actor_geometry(link,link.get_name(),planner)
    if len({s['name'] for s in shapes})!=len(shapes):raise ValueError('derived shape keys not unique')
    return {'schema_version':'cmf_retained_geometry_projection_v1','shapes':shapes,'geometry_sha256':digest(shapes)}

def run_remaining_scene(directory):
    from controlled_multi_future.planner_qualification_manifests_v2_3 import _f3_scene_binding
    from controlled_multi_future.real_sapien_adapter_high_level_v1 import _PinnedSapienRenderDeviceContextV1
    retained=load_json(W/'Robotwin2/datasets/f3_model_conformance_v1/f3-final-pose-v3-r3063/initial_geometry.json')
    recipes=candidates();scene_recipe=recipes[1];h=helper();legacy=candidate_spec(scene_recipe)
    adapter=h.adapter_for('F3',legacy,directory/'adapter',SOURCE_SHA)
    context=_PinnedSapienRenderDeviceContextV1(adapter.scene(legacy,phase='F3_REMAINING_MODEL_SCENE',program=None))
    scene=None;before=after=None;error=None;models=[];binding=None
    try:
        with context as handle:
            scene=handle.scene
            if not hasattr(scene,'planner_query_count'):scene.planner_query_count=0
            before=scene.planner_query_count
            try:
                binding=h.prepare_f3_scene(scene,adapter,scene_recipe,_f3_scene_binding(scene_recipe))
                names,_=runtime_joint_state(scene);planner=scene.robot.left_planner
                original_models={k:getattr(planner,k) for k in ('motion_gen','motion_gen_batch')}
                links={l.get_name():l for l in scene.robot.left_entity.get_links()}
                for recipe in recipes:
                    folder=directory/recipe['recipe_id'];states=old_states(recipe,names)
                    def exported(state):
                        project_state(scene,state);base_binding=bind_actual_solver_base(scene,planner)
                        e=replay_retained_geometry(retained,scene,state,planner) if recipe is recipes[0] else capture_scene_collision_geometry(scene,planner)
                        e['solver_base_binding']=base_binding;return e
                    initial=exported(states[0]);write_new(folder/'initial_geometry.json',initial)
                    _,_,derived,application=rebuild_actual_motiongens(scene,initial)
                    write_new(folder/'model_application.json',application);write_new(folder/'derived_robot_config.json',derived)
                    rows=[]
                    for state in states:
                        export=exported(state);locks=update_world_and_locks(planner,export,derived,names,np.asarray(state['qpos']))
                        row={'state':state,'locked_joints':locks,'geometry_sha256':export['geometry_sha256'],'old_models':{},'new_models':{}}
                        for name in ('motion_gen','motion_gen_batch'):
                            old_result,_=query_state(original_models[name],state['qpos'],names);mg=getattr(planner,name)
                            new_result,tensor=query_state(mg,state['qpos'],names)
                            actual_locks=dict(zip(new_result['full_model_joint_names'],new_result['full_model_qpos']))
                            if any(abs(actual_locks[n]-v)>1e-6 for n,v in locks.items()):raise ValueError('live solver finger state mismatch')
                            new_result['link_conformance']=link_conformance(scene,planner,mg,tensor)
                            new_result['nearest_robot_sphere_world_pairs']=closest_pairs(mg.kinematics,tensor,export)
                            row['old_models'][name]=old_result;row['new_models'][name]=new_result
                        fingers=[]
                        for name in ('fl_link6','fl_link7','fl_link8'):fingers+=capture_actor_geometry(links[name],name,planner)
                        row['exact_sapien_shape_pairs']=exact_shape_pairs(fingers,[s for s in export['shapes'] if s['role'] in ('table','pad','bottle')])
                        row['exact_geometry_intersection']=any(r['definite_open_grasp_geometry_violation'] and r['physical_collision_filter_enabled'] for r in row['exact_sapien_shape_pairs'])
                        write_new(folder/(state['label']+'.geometry.json'),export);write_new(folder/(state['label']+'.result.json'),row);rows.append(row)
                        print(recipe['recipe_id'],state['label'],{k:v['valid'] for k,v in row['new_models'].items()},'exact_intersection',row['exact_geometry_intersection'],flush=True)
                    clear,pre,endpoint=rows[0],rows[1],rows[-1]
                    conforms=all(r['new_models'][n]['valid'] for r in (clear,pre) for n in ('motion_gen','motion_gen_batch'))
                    conforms=conforms and all(p['position_error_m']<=.005 and p['orientation_error_rad']<=.05 for r in rows for n in ('motion_gen','motion_gen_batch') for p in r['new_models'][n]['link_conformance'])
                    decision='EXACT_ENDPOINT_GEOMETRY_INVALID_NO_PHYSICAL' if endpoint['exact_geometry_intersection'] else 'MODEL_CONFORMANCE_INCOMPLETE' if not conforms else 'ENDPOINT_VALID_MICRO_PREREQUISITES_PENDING' if all(v['valid'] for v in endpoint['new_models'].values()) else 'MODEL_REJECTED_ENDPOINT_GEOMETRY_REVIEW_REQUIRED'
                    result={'recipe_id':recipe['recipe_id'],'recipe_sha256':recipe['recipe_sha256'],'geometry_source':'retained asset13 capture' if recipe is recipes[0] else 'remaining live asset5 capture','states':rows,'model_conformance':bool(conforms),'decision':decision}
                    write_new(folder/'candidate_model_result.json',result);models.append(result)
            finally:after=getattr(scene,'planner_query_count',None)
    except BaseException as exc:error={'type':type(exc).__name__,'message':str(exc)}
    cleanup=context.cleanup_receipt;known=type(before) is int and type(after) is int and after>=before
    result={'schema_version':'cmf_f3_retained_plus_remaining_scene_v1_1','live_scene_recipe':scene_recipe['recipe_id'],'scene_binding':binding,'model_results':models,
        'render_device_binding':None if scene is None else getattr(scene,'_cmf_render_device_binding_v1',None),'error':error,'cleanup':cleanup,'accounting_complete':known,
        'planner_before':before,'planner_after':after,'trajectory_queries':after-before if known else None,'physical_attempts':0,'physical_actions_executed':False,
        'global_stop':not known or (known and after!=before) or error is not None or not isinstance(cleanup,dict) or cleanup.get('cleanup_safety_pass') is not True}
    write_new(directory/'scene_receipt.json',result);return result
