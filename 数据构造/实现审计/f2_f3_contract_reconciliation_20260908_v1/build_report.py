...
r=json.loads((B/'RECONCILIATION.json').read_text());cells=[c for p in sorted(B.glob('*_cells.json')) for c in json.loads(p.read_text())]
text='''# F2/F3 科学合同—实现—原始证据逐格影响报告
...
'''
for c in cells:
 f=c.get('f2');t=c.get('f3_terminal')
 if f:
  evidence=f"Z={f['final_actor_pose'][2]:.6f}m，末窗v={f['stable_linear_speed_max']:.3g}，stand接触={f['stand_top_contact_fraction']:.0%}"
  status='实际不符合beside；局部重采待决定' if '/beside/' in c['cell'] else 'CPU补出关系/稳定正证据；缺合同/观测项'
 else:
  evidence=f"共享V+3事件全过；瓶体回位误差={t['object_return_position_m']*1000:.3f}mm；EEF距row0={t['eef_to_row0_position_m']*1000:.2f}mm"
  status='事件/返回可保留；预定rest证据缺失'
 text+=f"| {c['cell']} | 七字段byte-identical | {evidence} | {status} |\n"
text+='''
## 对rest接线的处理
