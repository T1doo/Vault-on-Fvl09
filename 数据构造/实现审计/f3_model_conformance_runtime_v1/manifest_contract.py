...
W=Path('/nfs_share/lijunhui');A=W/'Vault-on-Fvl09/数据构造/实现审计';RUNTIME=Path(__file__).resolve().parent;BRIDGE=A/'f2_f3_model_bridge_v1'
...
def load_manifest(path,*,execution=True,runner=False,post=False):
    ...
    job=m['jobs'][0]
    for k,v in {'family':'F3','fresh_scene_cap':2,'trajectory_query_cap':0,'physical_attempt_cap':0,'states_per_scene':5,'timeout_seconds':3600}.items():
