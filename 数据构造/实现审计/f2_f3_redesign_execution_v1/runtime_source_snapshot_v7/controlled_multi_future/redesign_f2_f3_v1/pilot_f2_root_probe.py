...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--output", required=True); parser.add_argument("--root-id", required=True); parser.add_argument("--prefix-artifact"); parser.add_argument("--existing-root"); parser.add_argument("--programs", help="comma-separated programs to execute; remaining cells are reused from --existing-root"); parser.add_argument("--realizations", help="comma-separated realizations to execute; remaining cells are reused from --existing-root"); parser.add_argument("--qualification", action="store_true", help="run as non-collection transport qualification"); parser.add_argument("--build-prefix", action="store_true", help="build a fresh shared prefix from the first newly executed cell"); parser.add_argument("--layout-id", default="v1", choices=("v1", "v2", "v3")); args = parser.parse_args()
    ...
    result = run_root(args.output, args.root_id, Path(args.prefix_artifact) if args.prefix_artifact else None, Path(args.existing_root) if args.existing_root else None, programs, realizations, collection=not args.qualification, build_prefix=args.build_prefix, layout_id=args.layout_id)
