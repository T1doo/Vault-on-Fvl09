...
def run_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, seed: int, prefix_artifact: Path | None = None, collection: bool = True) -> dict[str, Any]:
    ...
    try:
        with managed_scene(RedesignF3Scene, args, receipt, f"f3-pilot-{program_id}-{realization_id}") as scene:
            ...
            grasp_offset = carried_object[:3] - carried_eef[:3]
            desired_object = np.asarray(anchor[:3], dtype=np.float64)
            return_support = carried_eef.copy(); return_support[:3] = desired_object - grasp_offset
            return_above = return_support.copy(); return_above[2] += 0.025
            receipt["return_target_binding"] = {"carried_eef": carried_eef[:3].tolist(), "carried_object": carried_object[:3].tolist(), "grasp_offset": grasp_offset.tolist(), "desired_object": desired_object.tolist(), "return_above": return_above[:3].tolist(), "return_support": return_support[:3].tolist()}
            ...
            final_object_speeds = np.asarray([np.linalg.norm(row["object_linear_velocity"]) for row in exit_rows], dtype=np.float64)
