...
def run(
    *,
    ...
    with np.load(trace_path, allow_pickle=False) as data:
        ...
        saved_qpos = np.asarray(data["joint_qpos"][prefix_end], dtype=np.float64)
        saved_qvel = np.asarray(data["joint_qvel"][prefix_end], dtype=np.float64)
        saved_object = np.asarray(data["object_pose"][prefix_end], dtype=np.float64)
        saved_eef = np.asarray(data["eef_pose"][prefix_end], dtype=np.float64)
    normalized_goals = []
    for item in goals:
