...
def run_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, seed: int, prefix_artifact: Path | None = None, collection: bool = True) -> dict[str, Any]:
    ...
    if realization_id == "r_pc":
        ...
        receipt["realization_contract"] = {"kind": "motion", "variant_applied": True, "path_variant": "direct_event_sequence", "motion_variant": "absolute_symmetric_planner_events_with_extended_holds"}
    else:
    ...
    try:
        with managed_scene(RedesignF3Scene, args, receipt, f"f3-pilot-{program_id}-{realization_id}") as scene:
            ...
            if prefix_artifact is None:
                ...
                # The 25 mm micro-lift is the grasp qualification gate, not
                # the F3 manipulation center.  Raise another 50 mm before
                # the symmetric +/-50 mm V so its negative half-cycle stays
                # clear of the support pad instead of clipping at ~20 mm.
                central = _eef(scene); central[2] += 0.05
                _move_arm(scene, central, "pilot_central_clearance"); receipt["action_segment_count"] += 1; _wait_and_record(scene, 50)
                receipt["central_clearance_lift_m"] = 0.05
...
def run_root(
    ...
    if existing_root is not None:
        ...
        source_prefix_artifact = source_root / "prefix_artifact.npz"
        if not source_prefix_artifact.is_file():
            raise ValueError("resume root is missing its immutable prefix artifact")
        prefix_artifact = root_output / "prefix_artifact.npz"
        shutil.copyfile(source_prefix_artifact, prefix_artifact)
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--output", required=True); parser.add_argument("--root-id", default="F3-A"); parser.add_argument("--existing-root"); parser.add_argument("--programs"); parser.add_argument("--realizations"); parser.add_argument("--cell-keys"); args = parser.parse_args()
    ...
    result = run_root(args.output, args.root_id, args.existing_root, programs, realizations, cell_keys); raise SystemExit(0 if result["accepted"] else 1)
