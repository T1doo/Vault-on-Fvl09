...
def run_cell(*, output: Path, root_id: str, program_id: str, realization_id: str, seed: int, prefix_artifact: Path | None = None, collection: bool = True) -> dict[str, Any]:
    output.mkdir(parents=True, exist_ok=False); receipt={"schema_version":"cmf_f2_pilot_cell_v1","root_id":root_id,"family":"F2","program_id":program_id,"realization_id":realization_id,"collection":collection,"formal_data":False,"stage0_data":False,"status":"running","action_segment_count":0,"solver_problem_count":0}; initialize_cleanup_fields(receipt); started=time.monotonic(); scene=None
    args=_args("F2",output/"scene"); args.update({"seed":seed,"task_name":f"cmf_f2_pilot_{root_id}","render_freq":0,"save_data":False,"collect_data":False,"need_plan":True})
    try:
        with managed_scene(RedesignF2Scene,args,receipt,f"f2-pilot-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if program_id == "inside":
                ...
                # Bind the EEF target to the carried can pose instead of
                # guessing a world-space EEF offset.  The grasp transform is
                # asset/pose-specific and is recorded in the cell receipt.
                carried_object = _pose(scene.can)
                grasp_offset = carried_object[:3] - carry[:3]
                extent_z = float(scene._cmf_redesign_asset_spec["scaled_extents"][2])
                desired_can = np.asarray([0.08, -0.18, 0.755 + extent_z / 2.0] if program_id == "on" else [0.23, -0.18, 0.825 + extent_z / 2.0], dtype=np.float64)
                target[:3] = desired_can - grasp_offset
                receipt["target_binding"] = {"carried_eef": carry[:3].tolist(), "carried_object": carried_object[:3].tolist(), "grasp_offset": grasp_offset.tolist(), "desired_can": desired_can.tolist(), "target_eef": target[:3].tolist()}
                # The prefix itself already carries the can above the wall
                # top. Keep that proven height for the branch lateral move;
                # an extra branch lift makes an otherwise reachable target
                # planner-infeasible.
            ...
            # A diagonal retreat clears the on-scale edge before rising; the
            # previous vertical-then-lateral route was planner-infeasible.
            exit_target=_eef(scene); exit_target[2]+=0.10; exit_target[0] += -0.15 if program_id == "on" else 0.10
            _move_arm(scene,exit_target,"pilot_empty_hand_exit",receipt=receipt); receipt["action_segment_count"]+=1; _wait_and_record(scene,70)
...
def run_root(
    ...
    run_programs = tuple(contract["program_ids"] if programs is None else programs)
    selected_cells(root_id, list(run_programs))
    if existing_root is not None and programs is None:
        raise ValueError("resume root requires an explicit program subset")
    ...
    if existing_root is not None:
        ...
        reuse_programs = [program for program in contract["program_ids"] if program not in set(run_programs)]
        reused_list = reusable_cells(source, root_id, reuse_programs)
    ...
    for cell in full_cells:
        ...
        if key in reused:
            ...
            continue
        if cell["program_id"] not in set(run_programs):
        ...
        receipt = run_cell(output=cell_dir, root_id=root_id, program_id=cell["program_id"], realization_id=cell["realization_id"], seed=contract["scene_seed"], prefix_artifact=artifact, collection=collection)
    ...
    receipt = {"schema_version": "cmf_f2_pilot_root_receipt_v1", "root_id": root_id, "family": "F2", "contract": contract, "cells": cells, "current_equivalence": len(current) == 1, "anchor_equivalence": len(anchors) == 1, "prefix_exact_replay": len(prefixes) == 1, "accepted": accepted, "status": "ACCEPTED" if accepted else "INCOMPLETE", "collection": collection, "collection_count": new_cell_count if collection else 0, "run_programs": list(run_programs), "new_cell_count": new_cell_count, "reused_cell_count": len(reused), "reused_from_root_receipt": source_receipt_sha, "reused_cell_keys": [f"{key[0]}:{key[1]}" for key in reused], "prefix_mode": "freshly_built_from_first_new_cell" if build_prefix else ("external_frozen_artifact" if external_prefix is not None else "cell_local")}
    atomic_write_json(root_output / "root_receipt.json", receipt)
    return receipt


def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--output", required=True); parser.add_argument("--root-id", required=True); parser.add_argument("--prefix-artifact"); parser.add_argument("--existing-root"); parser.add_argument("--programs", help="comma-separated programs to execute; remaining programs are reused from --existing-root"); parser.add_argument("--qualification", action="store_true", help="run as non-collection transport qualification"); parser.add_argument("--build-prefix", action="store_true", help="build a fresh shared prefix from the first newly executed cell"); args = parser.parse_args()
    programs = [item for item in args.programs.split(",") if item] if args.programs else None
    result = run_root(args.output, args.root_id, Path(args.prefix_artifact) if args.prefix_artifact else None, Path(args.existing_root) if args.existing_root else None, programs, collection=not args.qualification, build_prefix=args.build_prefix)
