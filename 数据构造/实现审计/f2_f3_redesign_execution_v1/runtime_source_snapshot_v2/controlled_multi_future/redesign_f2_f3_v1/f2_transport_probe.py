...
def run(output: str | Path, *, program_id: str = "on", realization_id: str = "r_pc", root_id: str = "F2-A", seed: int = 202609081) -> dict[str, Any]:
    root = Path(output); root.mkdir(parents=True, exist_ok=True)
    cell = run_cell(output=root / f"{program_id}_{realization_id}", root_id=root_id, program_id=program_id, realization_id=realization_id, seed=seed, prefix_artifact=None, collection=False)
    result = {
...
def main() -> None:
    ...
    parser.add_argument("--program", default="on", choices=("on", "beside"))
