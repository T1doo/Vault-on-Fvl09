...
class BudgetLedger:
    ...
    def __init__(self, path: str | Path, *, contract_sha256: str, task_id: str, parent_contract_sha256: str | None = None):
        self.path = Path(path)
    ...
    def _events_unlocked(self) -> list[dict[str, Any]]:
        ...
        with self.path.open(encoding="utf-8") as stream:
            for line_number, line in enumerate(stream, 1):
                ...
                allowed_contracts = {self.contract_sha256}
                if self.parent_contract_sha256:
                    allowed_contracts.add(self.parent_contract_sha256)
                if event_contract not in allowed_contracts:
                    raise LedgerError("ledger event belongs to another contract")
                if event_contract == self.parent_contract_sha256:
                    if seen_current_contract:
                        raise LedgerError("parent-contract event appears after budget amendment")
                else:
                    if not seen_current_contract and self.parent_contract_sha256:
                        if event.get("event_type") != "BUDGET_CAP_AMENDMENT":
                            raise LedgerError("new contract requires a budget amendment event")
                        metadata = event.get("metadata", {})
                        if metadata.get("parent_contract_sha256") != self.parent_contract_sha256:
