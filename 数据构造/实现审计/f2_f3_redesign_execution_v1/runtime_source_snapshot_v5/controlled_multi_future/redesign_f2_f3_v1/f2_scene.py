...
class RedesignF2Scene(DenseTraceMixin, AuditScene):
    ...
    def load_actors(self):
        ...
        self.box_center = np.asarray([-0.08, -0.18, 0.755], dtype=np.float64)
        scale_half = (0.06, 0.06, 0.005)
        stand_half = (0.05, 0.05, 0.035)
        if self._cmf_f2_layout_id in {"v2", "v3"}:
            ...
            self.box_center[0] = -0.12 if self._cmf_f2_layout_id == "v2" else -0.18
        ...
        spec = load_model_spec(asset_root, "071_can", model_id=0, asset_id="f2_can_v1")
        ...
        if self._cmf_f2_layout_id == "v2":
            self.scale_center = np.asarray([0.00, -0.18, 0.755], dtype=np.float64)
            self.stand_center = np.asarray([0.12, -0.18, 0.79], dtype=np.float64)
        elif self._cmf_f2_layout_id == "v3":
            self.scale_center = np.asarray([-0.02, -0.18, 0.755], dtype=np.float64)
            self.stand_center = np.asarray([0.08, -0.18, 0.79], dtype=np.float64)
        elif self._cmf_f2_layout_id == "v1":
            self.scale_center = np.asarray([0.08, -0.18, 0.755], dtype=np.float64)
            self.stand_center = np.asarray([0.23, -0.18, 0.79], dtype=np.float64)
        else:
        ...
        self._cmf_f2_box_contract = {
            ...
            "interior_upper_m": [float(self.box_center[0] + 0.11), float(self.box_center[1] + 0.11), 0.86],
