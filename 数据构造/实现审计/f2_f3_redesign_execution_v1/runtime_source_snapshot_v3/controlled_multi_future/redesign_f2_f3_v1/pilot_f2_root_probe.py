    ...
    try:
        with managed_scene(RedesignF2Scene,args,receipt,f"f2-pilot-{root_id}-{program_id}-{realization_id}") as scene:
            ...
            if program_id == "inside":
                ...
                # Bind the preplace center to the measured support top.  The
                # native can COM/geometry settles there in the qualification
                # trace; the dynamic contact verifier remains authoritative.
...
def run_root(
    ...
    receipt = {"schema_version": "cmf_f2_pilot_root_receipt_v1", "root_id": root_id, "family": "F2", "layout_id": layout_id, "contract": {**contract, "layout_id": layout_id}, "cells": cells, "current_equivalence": len(current) == 1, "anchor_equivalence": len(anchors) == 1, "prefix_exact_replay": len(prefixes) == 1, "accepted": accepted, "status": "ACCEPTED" if accepted else "INCOMPLETE", "collection": collection, "collection_count": new_cell_count if collection else 0, "run_programs": list(run_programs), "new_cell_count": new_cell_count, "reused_cell_count": len(reused), "reused_from_root_receipt": source_receipt_sha, "reused_cell_keys": [f"{key[0]}:{key[1]}" for key in reused], "prefix_mode": "freshly_built_from_first_new_cell" if build_prefix else ("external_frozen_artifact" if external_prefix is not None else "cell_local")}
...
def main():
    ...
    result = run_root(args.output, args.root_id, Path(args.prefix_artifact) if args.prefix_artifact else None, Path(args.existing_root) if args.existing_root else None, programs, collection=not args.qualification, build_prefix=args.build_prefix, layout_id=args.layout_id)
