...
def dispatch(*, vault_execution_dir: str | Path, output_root: str | Path, root_id: str = "F3-A", timeout_seconds: int = 1800, existing_root: str | Path | None = None, programs: list[str] | tuple[str, ...] | None = None, qualification: bool = False, build_prefix: bool = False, single_cell: tuple[str, str] | None = None, layout_id: str = "v1") -> dict[str, Any]:
    vault = Path(vault_execution_dir)
    if single_cell is not None and (not root_id.startswith("F2") or not qualification):
        raise ValueError("single-cell transport probe is only a non-collection F2 qualification")
    if single_cell is not None and (existing_root is not None or programs is not None):
    ...
    if existing_root is not None and programs is None:
        raise ValueError("a resume dispatch requires an explicit program subset")
    if single_cell is not None:
        run_programs = (single_cell[0],); cell_count = 1; suffix = "transport_probe"; dispatch_root_id = f"{root_id}-transport-{single_cell[0]}-{single_cell[1]}"
    else:
        run_programs = tuple(planned["program_id"] for planned in selected_cells(root_id, programs)) if programs is not None else tuple(planned["program_id"] for planned in expected_cells(root_id))
        # selected_cells returns one entry per realization; reservation is per cell.
        cell_count = len(selected_cells(root_id, programs)) if programs is not None else len(expected_cells(root_id)); suffix = "qualification" if qualification else ("resume_pilot" if existing_root is not None else "pilot"); dispatch_root_id = root_id
    ...
    if single_cell is not None:
        command = [sys.executable, "-m", "controlled_multi_future.redesign_f2_f3_v1.f2_transport_probe", "--output", str(output), "--root-id", root_id, "--program", single_cell[0], "--realization", single_cell[1], "--layout-id", layout_id]
    else:
        command = [sys.executable, "-m", child_module, "--output", str(output), "--root-id", root_id]
    if root_id.startswith("F2") and single_cell is None:
        if not build_prefix:
            command.extend(["--prefix-artifact", "/nfs_share/lijunhui/Robotwin2/datasets/f2_f3_redesign_v1/f2_qualification_prefix_artifact.npz"])
        if existing_root is not None:
            command.extend(["--existing-root", str(existing_root), "--programs", ",".join(sorted(set(run_programs)))])
        if qualification:
...
def main():
    parser = argparse.ArgumentParser(); parser.add_argument("--vault-execution-dir", required=True); parser.add_argument("--output-root", required=True); parser.add_argument("--root-id", default="F3-A"); parser.add_argument("--timeout-seconds", type=int, default=1800); parser.add_argument("--existing-root"); parser.add_argument("--programs", help="comma-separated programs to execute for a resume"); parser.add_argument("--qualification", action="store_true", help="run a non-collection transport qualification"); parser.add_argument("--build-prefix", action="store_true", help="build a fresh shared prefix from the first new cell"); parser.add_argument("--transport-probe", help="single-cell F2 qualification as program,realization"); parser.add_argument("--layout-id", default="v1", choices=("v1","v2","v3")); args = parser.parse_args()
    programs = [item for item in args.programs.split(",") if item] if args.programs else None
    single_cell = tuple(args.transport_probe.split(",", 1)) if args.transport_probe else None
    result = dispatch(vault_execution_dir=args.vault_execution_dir, output_root=args.output_root, root_id=args.root_id, timeout_seconds=args.timeout_seconds, existing_root=args.existing_root, programs=programs, qualification=args.qualification, build_prefix=args.build_prefix, single_cell=single_cell, layout_id=args.layout_id)
