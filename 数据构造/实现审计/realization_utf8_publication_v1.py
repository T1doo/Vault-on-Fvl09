...
from catalog import sha,seal
from realization_utf8_io_v1 import write_new,load_json

def publish_identical_or_new(path,value):
    if Path(path).exists():
        if load_json(path)!=value:raise ValueError('immutable publication differs')
...
def audit_completed_root(directory):
    directory=Path(directory);root=load_json(directory/'root_receipt.json')
    ...
    for embedded in root['branch_receipts']:
        path=directory/'branches'/embedded['program_id']/'receipt.json';actual=load_json(path)
