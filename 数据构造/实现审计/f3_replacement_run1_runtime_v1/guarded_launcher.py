#!/usr/bin/env python3
"""Guard wrapper for the approved F3 centralized replacement Gate."""
...
BASE_GUARD = Path("/nfs_share/lijunhui/Robotwin2/production_micro_gate_v1/guarded_launcher.py")
...
def load_base_guard():
    ...
    spec = importlib.util.spec_from_file_location("cmf_f3_replacement_base_guard", BASE_GUARD)
...
def main(argv=None):
    ...
    if args.physical_index is None or args.expected_uuid is None:
        raise ValueError("F3 runtime Guard requires GPU index and UUID")
    load_manifest(args.manifest, args.job_id, phase="guard")
    base = load_base_guard()
    base.load_manifest = lambda path: load_manifest(Path(path), args.job_id, phase="guard")[0]
    return base.main(["--manifest", str(args.manifest), "--job-id", args.job_id, "--physical-index", str(args.physical_index), "--expected-uuid", args.expected_uuid])
