...
def main(argv=None):
    ...
    if a.preflight_only:
        tree=ast.parse(Path(__file__).with_name('micro.py').read_text(encoding='utf-8'));assert not any(isinstance(n,ast.Attribute) and n.attr in ('run_nonformal_root','shared_v') for n in ast.walk(tree));print('one explicitly approved replacement; actual-state plans; unchanged physical gates')
        import unittest
        from test_lifecycle import Tests
