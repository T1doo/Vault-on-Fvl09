...
def load_manifest(path,*,execution=True,runner=False,post=False):
    ...
    job=m['jobs'][0]
    for k,v in {'family':'F3','fresh_scene_cap':2,'trajectory_query_cap':0,'physical_attempt_cap':0,'ik_problem_cap':6,'timeout_seconds':3600}.items():
...
def load_and_validate_manifest_job(path,job_id,*,phase,require_execution_authorized,executable_role,executable_path):
    ...
    if phase==GUARD_ENTRY:
        ...
        if terminal['manifest_sha256']!=m['manifest_sha256'] or terminal['scene_attempts']>2 or terminal['physical_attempts']!=0:raise ValueError('terminal scope')
        if terminal['accounting_complete'] and (terminal['trajectory_queries']!=0 or terminal['IK_problems']>6):raise ValueError('unapproved trajectory query')
