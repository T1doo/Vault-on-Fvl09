...
def witness():return {'only_attached_bottle_support_pairs_overlap':True,'hold':{'frames':250,'all_selected_contact':True,'all_both_fingers':True,'all_signal_complete':True,'forbidden_count':0,'support_contact_frames':250,'max_relative_translation_drift_m':.0001,'max_relative_orientation_drift_rad':.001}}
class Backend:
...
class Tests(unittest.TestCase):
    def make(self):
        ...
        return PairFilteredWorld(self.full,self.sub,sphere_link_names=['fl_link7','attached_bottle','fl_link1'],attached_link_name='attached_bottle',support_name='pad__0',witness=witness(),buffer_factory=lambda q:object())
    ...
    def test_dropping_another_obstacle_rejected(self):
        ...
        with self.assertRaises(ValueError):PairFilteredWorld(self.full,self.sub,sphere_link_names=c.names,attached_link_name=c.attached,support_name='pad__0',witness=witness(),buffer_factory=lambda q:None)
    def test_no_bad_contact_witness(self):
        c=self.make();w=witness();w['hold']['forbidden_count']=1
        with self.assertRaises(ValueError):PairFilteredWorld(self.full,self.sub,sphere_link_names=c.names,attached_link_name=c.attached,support_name='pad__0',witness=w,buffer_factory=lambda q:None)
