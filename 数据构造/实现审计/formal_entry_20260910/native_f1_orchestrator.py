"""Isolated F1 recovery version of root_orchestrator_v1_2; original unmodified.
CPU changes: fail-fast branches, immutable raw reuse after all new bindings agree.
Recovery repeats qualification/planner stages and must budget their real cost.
"""
...
class FormalF1RecoverableOrchestrator(
    RealSapienPilotRootOrchestratorV1_1
):
        ...
        try:
            ...
            for program in programs:
                ...
                if reuse is not None:
                    ...
                    old_suffix, old_arrays, _ = load_frozen_suffix_artifact(old_suffix_dir)
                    _, new_arrays, _ = load_frozen_suffix_artifact(suffix_artifact_dirs[program_id])
                    suffix_equal = set(old_arrays) == set(new_arrays) and all(old_arrays[k].dtype == new_arrays[k].dtype and old_arrays[k].shape == new_arrays[k].shape and old_arrays[k].tobytes() == new_arrays[k].tobytes() for k in old_arrays)
                    reuse_checks = {
                        'accepted': saved.get('status') == 'accepted',
                        'program': saved.get('program_id') == program_id,
                        'current': saved.get('reference_current_sha256') == reference_current['aggregate_sha256'],
                        'candidates': saved.get('candidate_universe_sha256') == frozen['candidate_universe_sha256'],
                        'prefix': saved.get('prefix_artifact_sha256') == manifest['artifact_sha256'],
                        'suffix_actual_arrays': suffix_equal,
                        'suffix_targets': old_suffix['execution_spec']['targets'] == suffix_manifest['execution_spec']['targets'],
                    }
                    if not all(reuse_checks.values()):
                        raise ValueError(f'reused branch differs from regenerated bindings: {reuse_checks}')
                ...
                if branch.get('status') != 'accepted':
                    terminal = 'failed_verifier'
                    raise RuntimeError('first branch failed; later branch execution stopped')
