...
def recovery_cpu_preflight(*, manifest, job, state_dir, state, request):
    ...
    _validate_resume_source_identity(attempt_number=attempt_number, checkpoint=checkpoint, compatibility=compatibility)
...
def _validate_resume_source_identity(*, attempt_number, checkpoint, compatibility):
    """Require a recovery amendment to start from the source actually sealed in the root checkpoint.

    A previous authorization can describe a source candidate that never reached
    the root.  The checkpoint is the execution-side source token, so it is the
    only valid old implementation identity for motion recovery attempts after
    the initial source transition.
    """
    if int(attempt_number) < 5:
        return True
    if compatibility.get('old_source_sha256') != checkpoint.get('source_sha256'):
...
def launch_wave(manifest,state_dir,backend=None,*,ready_job_ids=None,recovery_requests=None,copy_only_job_ids=None,activated_jobs=None,compatibility_ref=None):
    ...
    with (state_dir/'coordinator.lock').open('a') as coordinator:
        ...
        def explicit_attempt7_exception(job_id, record):
            request = requests.get(job_id)
            attempts = record.get('attempts') or []
            contract = manifest.get('recovery_contract') or {}
