...
def load_manifest(path,*,execution=True,runner=False,post=False):
    ...
    auth=checked(m['authorization_path'])
    if not auth['approved'] or auth['receipt_sha256']!=m['authorization_receipt_sha256'] or auth['constraint_checks']!=4 or auth['trajectory_queries']!=1:raise PermissionError('new replay approval binding')
    ...
    if m['physical_execution_authorized'] is not False:raise PermissionError('no physical actions allowed')
    previous=checked(m['previous_terminal_path']);guard=checked(m['previous_guard_path'])
    if not previous['pass'] or previous['micro_pass'] or not guard['task_owned_cleanup_pass']:raise ValueError('saved postclose provenance/cleanup')
    ...
    job=m['jobs'][0]
    for k,v in {'family':'F3','fresh_scene_cap':0,'trajectory_query_cap':1,'physical_attempt_cap':0,'ik_problem_cap':0,'timeout_seconds':1800}.items():
...
def load_and_validate_manifest_job(path,job_id,*,phase,require_execution_authorized,executable_role,executable_path):
    ...
    if phase==GUARD_ENTRY:
        ...
        if terminal['manifest_sha256']!=m['manifest_sha256'] or terminal['scene_attempts']!=0 or terminal['physical_attempts']!=0:raise ValueError('terminal scope')
        if terminal['accounting_complete'] and (terminal['trajectory_queries']>1 or terminal['IK_problems']!=0 or terminal['constraint_checks']>4):raise ValueError('unapproved trajectory query')
